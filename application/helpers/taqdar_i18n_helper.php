...
/* ====================================================================
   TQ-I18N-SURFACE — سطحان لا سطح واحد
   ==================================================================== */

/**
 * مبدل اللغة يعيش في ترويسة اللوحات الأربع وحدها — لا في ترويسة الموقع
 * العام. فمن بدل لغته إنما بدلها **حيث يعمل**: في لوحته. وكان التبديل يقع
 * على الطلب كله، فيخرج المسؤول من لوحته إلى الرئيسية فيجدها إنجليزية
 * مفتوحة من اليسار — صفحة تسويق كتب نصها العربي بيد، وصورها ومسافاتها
 * مصممة على اتجاه واحد، **وليس فيها زر واحد يرجعه**. فهو يقلب لغة لوحته
 * ذهابا وإيابا ليقرأ الموقع كما كان.
 *
 * والسطحان ليسا شكلين لشيء واحد:
 *
 *   اللوحة  — أداة عمل يقرؤها صاحبها كل يوم، ولغتها **تفضيله هو**.
 *   الموقع  — واجهة المنصة إلى من لا حساب له، ولغتها **قرار منصة**
 *             (`settings.language`) لا قرار زائر: لا مبدل فيها يقلبها،
 *             ولا نص فيها يخاطب مستخدما بعينه.
 *
 * فاللغة تحل بالسطح: في اللوحة تقرأ الجلسة وتفضيل الحساب وكعكة الزائر،
 * وفي الموقع تسقط إلى لغة المنصة مباشرة. وهي قاعدة «بلا مفتاح لا شيء
 * يتغير» نفسها: من لم يبدل شيئا يرى ما كان يرى حرفا بحرف.
 */

if (!function_exists('tq_i18n_panel_uris')) {
    /**
     * المقاطع الأولى التي تعني «لوحة».
     *
     * والمقارنة على **مقطع الرابط كما كتبه المتصفح** لا على اسم المتحكم
     * المسير إليه: `Taqdar.php` يخدم البوابات الثلاث **وصفحات الموقع
     * العام معا** (`/catalog` · `/plans` · `/plan/<code>` · `/checkout`)،
     * فاسم صنفه لا يفرق بين سطح وسطح.
     *
     * @return array<string, bool>
     */
    function tq_i18n_panel_uris()
    {
        return array(
            'taqdar_admin' => true,   // لوحة الإدارة
            'admin'        => true,   // شاشات Academy الموروثة داخلها
            'student'      => true,   // البوابات الثلاث
            'teacher'      => true,
            'parent'       => true,
            'taqdar_gate'  => true,   // نداءات البوابة (JSON)
            'taqdar'       => true,   // البادئة القديمة، محولة بـ301
            'user'         => true,   // لوحة المحاضر الموروثة، محولة بـ301
        );
    }
}

if (!function_exists('tq_i18n_surface')) {
    /**
     * سطح هذا الطلب — `'panel'` أو `'site'`.
     *
     * ويحل **مرة واحدة**: `tq_lang()` تنادى مئات المرات في الصفحة، وقراءة
     * الرابط في كل نداء عمل يعاد بلا سبب.
     *
     * @param string|false|null $set  `'panel'`/`'site'` لتثبيته، أو `false` لتصفيره
     */
    function tq_i18n_surface($set = null)
    {
        static $surface = null;

        if (func_num_args()) {
            $surface = ($set === false) ? null : (string) $set;
            tq_i18n_state(false);   // اللغة نتيجة السطح، فتصفر معه
            return $surface;
        }
        ...
        /* بلا نسخة CI لا رابط يقرأ — ولا يخزن الجواب: النداء الذي يسبق
           بناء المتحكم لا يجوز أن يقرر سطح الطلب كله بعده. */
        if (!$CI || !isset($CI->uri)) return 'site';

        $uri = trim((string) $CI->uri->uri_string(), '/');

        /* و`explode` لا `strtok`: الثانية تحمل حالة عامة، فنداؤها هنا
           يقطع تجزئة يجريها غيرنا في الطلب نفسه. */
        $parts = explode('/', $uri, 2);
        $seg   = strtolower($parts[0]);

        /* استثناء الاصطلاح: التحقق من شهادة يفتح بلا حساب — يفتحه من
           يتحقق من وثيقة: جهة توظيف أو مدرسة. فهو صفحة موقع وإن حملت
           البادئة القديمة. وهو الاستثناء نفسه المكتوب في غلاف الثيم
           (`$tq_public_tq_pages`). */
        if ($seg === 'taqdar' && strpos(strtolower($uri), 'taqdar/verify') === 0) {
            return $surface = 'site';
        }

        $panels = tq_i18n_panel_uris();
        return $surface = (isset($panels[$seg]) ? 'panel' : 'site');
    }
}

if (!function_exists('tq_is_panel')) {
    /** أهذا الطلب على سطح لوحة؟ */
    function tq_is_panel() { return tq_i18n_surface() === 'panel'; }
}
...
if (!function_exists('tq_lang')) {
     ...
     *   ٤) إعداد المنصة.
     *
     * **والثلاثة الأولى تفضيل شخصي، فلا تقرأ إلا على سطح اللوحة**
     * (TQ-I18N-SURFACE): المبدل يعيش في ترويسة اللوحات وحدها، والموقع
     * العام لا زر فيه يقلب لغته ولا يرجعها — فلغته لغة المنصة.
     *
    ...
    function tq_lang()
    {
        ...
        /* الموقع العام: لغة المنصة، ولا يسأل عن تفضيل أحد.
           والكتابة في الحامل هنا كما تكتب في كل فرع — فلا يعاد القرار
           في كل نداء من مئات نداءات `t()` في الصفحة. */
        if (tq_i18n_surface() !== 'panel') {
        ...
        /* ٢ — تفضيل الحساب.
              الجدول ينشأ وقت التشغيل فقد لا يكون بعد، فالاستعلام ملفوف:
              استعلام على جدول غائب يرمي استثناء يبيض الصفحة، والافتراض أهون.

              ولا يفحص بـ`table_exists()`: هي تقارن الاسم بقائمة
              `list_tables()` بعد أن تمرره على `protect_identifiers()`، وترد
              **كاذبة على جدول قائم** في هذا التركيب — فيسقط الفرع كله بلا
              خطأ، ويقرأ من بدل لغته على جهاز عربيته على الجهاز الثاني
              والتفضيل محفوظ في الصف لا يقرؤه شيء. والاستعلام نفسه أصدق:
              إن كان الجدول غائبا رمى، والرمي ملتقط. */
...
/* ====================================================================
   نقاط الاختناق — ترجمة ما ولد تحت القوالب
   ==================================================================== */

/**
 * TQ-I18N-SINK — الرسالة تترجم حيث تعرض، لا حيث تكتب.
 *
 * رسائل المنصة تولد في النماذج والمتحكمات: مئة وأربع وعشرون
 * `set_flashdata` وحدها، ومثلها في ردود البوابة وأخطاء التحقق. ولف كل
 * واحدة منها بـ`t()` عند الكتابة يعني تعديل ثلاثين ملفا في الطبقة التي
 * تحمل قواعد العمل — وأخطر منه أنه **يترجم في الوقت الخطأ**: مهمة الكرون
 * تكتب إشعارا بلغة الخادم لا بلغة صاحبه، والرسالة تخزن مترجمة فيقرؤها
 * من بدل لغته بعدها بلغة لم يعد يقرؤها.
 *
 * ومفتاح القاموس هو **النص العربي نفسه** (انظر رأس هذا الملف)، فالترجمة
 * عند العرض تعمل بلا أن تمس تلك الطبقة بسطر: الرسالة تصل عربية كما كتبت،
 * وتخرج بلغة من يقرؤها الآن.
 */
if (!function_exists('tq_flash')) {
    /** يقرأ رسالة طائرة ويترجمها — والمصفوفة تترجم عنصرا عنصرا. */
    function tq_flash($key)
    {
        $CI = get_instance();
        if (!isset($CI->session)) return null;
        return tq_t_deep($CI->session->flashdata($key));
    }
}

if (!function_exists('tq_t_deep')) {
    /**
     * يترجم نصا أو شجرة نصوص.
     *
     * وأخطاء التحقق تصل **مصفوفة** (`fail($errors, $section)`)، ومفاتيحها
     * أسماء حقول لا نص — فالمفاتيح تترك والقيم تترجم. ولو ترجمت المفاتيح
     * لما وجدت الشاشة خطأ الحقل الذي تسأل عنه، فيحفظ نموذج ناقص بلا رسالة.
     *
     * @param mixed $v
     * @param array $skip مفاتيح تمر قيمها كما هي مهما غاصت
     */
    function tq_t_deep($v, $skip = array())
    {
        if (is_string($v)) return t($v);
        if (is_array($v)) {
            $out = array();
            foreach ($v as $k => $x) {
                /* مفتاح مستثنى: قيمته تمر كما هي مهما غاصت.
                   و`default` في سجل نصوص الموقع مثاله — تلك **قيمة تكتب في
                   القاعدة** لا تسمية تعرض، وترجمتها تجعل المسؤول يحفظ نصا
                   إنجليزيا على صفحة عربية وهو يظن أنه لم يغير شيئا. */
                $out[$k] = in_array($k, $skip, true) ? $x : tq_t_deep($x, $skip);
            }
            return $out;
        }
        return $v;
