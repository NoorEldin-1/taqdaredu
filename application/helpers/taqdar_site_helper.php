...
if (!function_exists('tqs_nav')) {
    ...
    function tqs_nav($active = '')
    {
        /* «المواد والبرامج» قبل «الباقات»: الزائر يتصفح قبل أن يشتري،
           ...
           TQ-BOOK — وبند «كتبي» عاد، وليس نقضا لما قبله. حذف يوم كان
        ...
        $items = array(
            ...
            /* TQ-SUM-DRIVE — بند واحد للكتب والملخّصات، ومبدّل داخل
               الصفحة يفصلهما. كانا بندين فطالا القائمة بموضوع واحد:
               من يبحث عن مادّة صفّه يقرأ عنوانين متجاورين ويقف بينهما.
               والفصل يبقى — لكنّه صار حيث يُقرأ: تبويبان بعدّادين فوق
               الشبكة، بنمط `cactive` نفسه الذي تعرفه صفحة الصفّ. */
            array('books',    'books',     t('كتبي وملخصاتي')),
...
if (!function_exists('tqs_nav_key')) {
    ...
    function tqs_nav_key($page_name)
    {
        $map = array(
            ...
            /* TQ-SUM-DRIVE — والملخّص صفحته `site_book` نفسها (هو صفّ في
               `books`)، فلا مفتاح ثانٍ له: من فتح ملخّصًا من قسمه يرى
               «كتبي» مضاءً لا «الملخّصات». وهذا حدّ مقبول: البند يضيء
               للقسم لا لكلّ ما يُفتح منه. */
            'site_summaries' => 'books',
