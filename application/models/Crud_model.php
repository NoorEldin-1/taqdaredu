    ...
    public function get_total_duration_of_lesson_by_course_id($course_id)
{
    ...
    /**
     * SECURITY 2026-08-02 - the old token was
     *   base64(email . '--' . rand(1111,9999))
     * i.e. only ~8900 guessable values for a known e-mail address, with no
     * attempt limit. It is now 32 cryptographically random bytes (64 hex
     * chars), stored only as a sha256 fingerprint in tq_auth_password_resets,
     * valid for TQ_AUTH_RESET_TTL and usable exactly once.
     */
    function forgot_password()
    {
        ...
        // keep the historical "a request was made at" marker
    ...
    {
        // Burns the token first (atomic UPDATE ... WHERE used_at IS NULL), so a
        // replayed reset link cannot set the password a second time.
        $reset_request = tq_auth_consume_reset_token($verification_code);
