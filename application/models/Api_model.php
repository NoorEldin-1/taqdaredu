...
class Api_model extends CI_Model
{
	...
	public function login_get()
	{
		...
		if (tq_auth_is_throttled($email, 'api_login')) {
			$userdata['validity'] = 0;
			$userdata['device_verification'] = 'invalid-login-credentials';
			$userdata['message'] = tq_auth_throttle_message();
			return $userdata;
		}
		...
		if ($query->num_rows() > 0 && tq_password_authenticate($query->row_array(), $password)) {
			$row = $query->row_array();
			tq_auth_clear_failures($email, 'api_login');
			...
			tq_auth_record_failure($email, 'api_login');
			$userdata['validity'] = 0;
            $userdata['device_verification'] = 'invalid-login-credentials';
		}
		return $userdata;
	...
	public function signup_post()
	{
	    ...
	    $data['password'] = tq_password_hash($_POST['password']);
