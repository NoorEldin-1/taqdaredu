...
class Taqdar_billing_model extends CI_Model
{
    ...
    public function sync_enrolments($subscription_id)
    {
        ...
        /* TQ-NELC — يبلغ المركز الوطني بتسجيل الطالب في كل كورس يفتحه
           هذا الاشتراك. وعلى `$courses` كلها لا على ما أدرج الآن: صف
           `enrol` قد يكون كتب قبل أن يفتح التكامل أصلا، وصاحبه مسجل
           فعلا. والتكرار يمنعه `event_key`، فالتجديد لا يعد تسجيلا ثانيا.

           وملفوف لأن فشل الإبلاغ لا يبطل استحقاقا دفع ثمنه — ومعه
           `reset_query()` (TQ-BUILDER-DIRTY). */
        try {
            $this->load->model('taqdar_nelc_model', 'tq_nelc');
            if ($this->tq_nelc->ready()) {
                foreach ($courses as $cid) $this->tq_nelc->registered($uid, $cid);
            }
        } catch (Throwable $e) {
            $this->db->reset_query();
            log_message('error', 'TQ-NELC sync_enrolments: ' . $e->getMessage());
        }
