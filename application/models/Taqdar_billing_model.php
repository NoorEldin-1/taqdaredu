...
class Taqdar_billing_model extends CI_Model
{
    ...
    public function subscribe_path($user_id, $path_id, $method = "manual")
    {
        ...
        /* TQ-PLACEMENT — رفع هنا كما رفع عن شراء الباقة (قرار المالك
           ٢٠٢٦-٠٨-٢٩): بابان للشراء ورفع عن أحدهما وحده تناقض — من فتح
           له باب الباقة يجب أن يفتح له باب المسار. والاختبار يبقى في
           لوحة الطالب يؤخذ بعد الشراء لا قبله. */
    ...
    public function subscribe($user_id, $plan_id, $method = 'manual')
    {
        ...
        /* TQ-PLACEMENT — رفع بقرار المالك (٢٠٢٦-٠٨-٢٩): الاختبار
           التشخيصي **يشخص ولا يمنع بيعا**.

           كان يحجب الشراء عمن لصفه اختبار منشور — والصف الأول الابتدائي
           وحده كذلك اليوم. والمشتري في هذه المنصة **ولي الأمر لا الطالب**:
           فطلب اختبار تحديد مستوى ممن يدفع، قبل أن يدفع، يوقف بيعا بلا
           سبب — والاختبار ليس له أصلا.

           والاختبار لم يلغ: `Taqdar::student()` ما زال يطالب به الطالب في
           لوحته (`Taqdar.php` — حارس `placement_open`)، فيؤخذ بعد الاشتراك
           حيث ينفع: يحدد موضع الطالب في محتوى صار يملكه.

           ولا شيء آخر تغير: `gate()` كما هي، و`Taqdar_diag_model` كما هو،
           وشاشة `student/placement` تعمل. المرفوع هو الحجب عن الشراء وحده. */
