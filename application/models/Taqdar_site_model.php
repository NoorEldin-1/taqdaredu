...
/**
 * قراءة المحتوى العامّ — للزائر المجهول.
 *
 * قبل هذا الملفّ كان الاستعلام الوحيد الصالح للكتالوج مكتوبًا **داخل عرض**
 * (`plans.php`)، فلا يُعاد استعماله ولا يُختبر ولا يُصلَح إلّا في مكانه.
 * وهنا مصدرٌ واحد تقرأ منه الصفحات كلّها، فلا يفترق ما تعرضه صفحةٌ عمّا
 * تعرضه أختها.
 *
 * وكل دالّة هنا تفترض **زائرًا بلا حساب**: لا شيء يعتمد على جلسة، ولا صفّ
 * غير منشور يخرج منها.
 */
class Taqdar_site_model extends CI_Model
{
    /** الفئات المنشورة مرتّبة، بالشكل الذي تتوقّعه مُولِّدات الوسم. */
    public function categories()
    {
        /* العدّ من `paths` لا من `course`: صفحة الأقسام كانت تعدّ دورات
           Academy، وفي القاعدة دورة واحدة بلا قسم — فتقول كل قسم «صفر»
           بينما الكتالوج يعرض خمسة عشر مسارًا. كيانان يُعدّان في صفحتين. */
    ...
    /**
     * المسارات المنشورة بوصفها «موادّ» الكتالوج.
     *
     * عدد الدروس والاختبارات يُحسب من الدورة المرتبطة — ولا يُخزَّن رقمًا
     * في المسار. الرقم المخزَّن يشيخ في صمت: يُكتب مرّة ثم يبقى بعد أن
     * يتغيّر ما يصفه. وحين لا دورة، يعود صفرًا والعرض يقول «قريبًا» بدل
     * أن يَعِد بمحتوى لا وجود له.
     */
    ...
    /** مسار واحد بمسمّاه، ومعه فئته ومعلّمه ودورته. */
    public function path_by_slug($slug)
    {
        $this->db->select('p.*, c.name AS cat_name, c.slug AS cat_slug,
                 ...
                 ->where('p.status', 'published');
        /* شرط حالة الدورة في الضمّ لا في `where`: مسارٌ بلا دورة منشورة
           يبقى معروضًا (بلا زرّ شراء)، لكن عنوان دورةٍ قيد المراجعة
           لا يُعرَض لزائر — وقد كان `[UPTEST]` يظهر منه. */
    ...
    /**
     * المعلّمون المعروضون علنًا.
     *
     * العرض العامّ اختيارٌ صريح (`is_public`) لا أثرٌ جانبيّ لكون المستخدم
     * معلّمًا: من يدرّس لا يلزم أن يُنشر اسمه وصورته على صفحة عامّة.
     */
    ...
    /** المقالات المنشورة، وأوّل المميَّز منها إن وُجد. */
    ...
    /** تصنيفات المدوّنة التي لها مقال منشور واحد على الأقلّ. */
    ...
    /** مسابقاتٌ مفتوحة، الأقرب موعدًا أوّلًا. */
    ...
    /** أحدث الدروس المنشورة عبر المسارات — «القادم» يعني الجديد لا المجدوَل. */
    ...
    /** مسارات مميّزة: ما له دورة موصولة أوّلًا — فلا نُبرز ما لا يُفتح. */
    ...
    /**
     * بحثٌ في المحتوى العامّ.
     *
     * **بلا استعلامٍ جديد**: يُعيد استعمال `materials()` و`posts()`
     * و`books()` و`teachers()` ثمّ يُرشِّح في PHP. والمجموع بضع عشرات من
     * الصفوف، فالفرق في الأداء لا يُقاس — والمكسب أنّ نتيجة البحث هي
     * **نفسها** ما تعرضه الصفحات: لا استعلامٌ ثانٍ يُنقّح شرطَ نشرٍ أو
     * ترتيبًا ثمّ يتباعد عنها، ولا نصٌّ من المستخدم يدخل SQL.
     */
    ...
    /**
     * الباقة كاملةً بمحتواها — مصدرُ الحقيقة الواحد لصفحاتها الثلاث.
     *
     * صفحة الباقة العامّة، وشاشة تأكيد الشراء، وصفحة «محتوى باقتي» للمشترك
     * تقرأ كلّها من هنا. ولولا ذلك لافترقت الأرقام: صفحةٌ تَعِد بمئة درس
     * وصفحةٌ تعرض ثمانين، والفرق سطرُ استعلامٍ نُسِخ ثم شاخ.
     *
     * وكلّ عددٍ هنا **يُحسب من القاعدة لحظةَ الطلب** — لا رقم مخزَّن ولا
     * رقم مكتوب في العرض. فما لم يُستورد بعدُ يظهر صفرًا، والعرض يُخفي
     * الصفر بدل أن يَعِد بما لا وجود له.
     *
     * خمسة استعلاماتٍ مهما كبرت الباقة (لا استعلامٌ لكلّ برنامج):
     *   الباقة · الصفوف · البرامج · الوحدات · الدروس
     *
     * @param  string $code  رمز الباقة (`plans.code`) أو رقمها
     * @return array|null    null إن لم توجد أو كانت معطّلة
     */
    public function bundle_by_code($code)
    {
        ...
        /* ٢ · الصفوف ------------------------------------------------- *
           `scope_ids` قائمةٌ بفواصل لأنّ الباقة تشمل مرحلةً كاملة، و
           `scope_id` رقمٌ واحد. وما عدا `grade` لا يُقيَّد بصفّ: الباقة
           المجّانية والشاملة تفتحان الكتالوج المنشور كلَّه. */
        $gids = array();
        if ($b['scope'] === 'grade') {
            ...
            if (!$gids) return $b;   // باقةُ صفوفٍ بلا صفوف: لا محتوى تصفه
        }
        ...
        /* ٤ · الوحدات — أوّل قراءةٍ لـ`section` في واجهةٍ للمستخدم ----- *
           كانت `ordered_lessons()` تضمّها وتُرجع عنوانها، ثمّ يُهدَر في كلّ
           عرض. والمنهج بلا وحداتٍ قائمةٌ مسطّحةٌ من ثمانين سطرًا لا يُقرأ. */
        ...
        /* ٥ · الدروس ------------------------------------------------- */
        $byCourse = array();   // course_id → إحصاء + دروسٌ بلا وحدة
        if ($cids) {
            ...
            foreach ($rows as $r) {
                ...
                /* درسٌ بلا وحدة ليس خطأً يُسكَت عنه: يُجمع في وحدةٍ ضمنيّة
                   تُسمّى «الدروس» فيظهر بدل أن يسقط من المنهج صامتًا. */
        ...
        foreach ($paths as $p) {
            ...
            if ($cid > 0 && isset($units[$cid])) {
                foreach ($units[$cid] as $u) {
                    if (!$u['lessons']) continue;        // وحدةٌ فارغة لا تُعرض
                    $uts[] = $u;
                }
            }
            if ($stat['loose']) {
        ...
        /* صفٌّ بلا برنامجٍ منشور لا يُعرض: الباقة تَعِد بما تفتحه فعلًا */
