...
class Api_instructor_model extends CI_Model
{
	...
	public function login_post()
	{
		...
		// Brute force throttle (same table/window as the web login).
		if (tq_auth_is_throttled($email, 'api_instructor_login')) {
			$response['status']   = 429;
			$response['validity'] = 0;
			$response['message']  = tq_auth_throttle_message();
			return $response;
		}

		// Fetch by e-mail, then verify the hash so that both legacy sha1 and
		// password_hash() digests are accepted (and legacy ones get upgraded).
		$credential = array('email' => $email, 'is_instructor' => 1, 'role_id' => 2, 'status' => 1);
		$query = $this->db->get_where('users', $credential);
		if ($query->num_rows() > 0 && tq_password_authenticate($query->row_array(), $password)) {
			tq_auth_clear_failures($email, 'api_instructor_login');
			$row = $query->row_array();
			...
			tq_auth_record_failure($email, 'api_instructor_login');
			$response['status'] = 403;
			$response['validity'] = 0;
	...
	public function change_password_post($user_id = "")
	{
		...
		if ($query->num_rows() > 0) {
			...
			if (tq_password_matches($current_password, $user_details['password']) && $new_password == $confirm_password) {
				$data['password'] = tq_password_hash($new_password);
				$this->db->where('id', $user_id);
				$this->db->update('users', $data);
	...
	function forgot_password_post(){
        ...
        // SECURITY 2026-08-02: was base64(email.'_Uh6#@#6hU_'.rand(111111,9999999)),
        // guessable. Now a single use random_bytes(32) token, see config.php.
        if (tq_auth_is_throttled($email, 'reset_request')) {
            return false;
        }
        tq_auth_record_failure($email, 'reset_request');

        $verification_code = tq_auth_create_reset_token($email);
        if ($verification_code === FALSE) {
            return false;
        }

        $this->db->where('email', $email);
        $this->db->update('users', array('last_modified' => time()));
