...
class Payment_model extends CI_Model
{
                                   ...
                                   $client_id = "", $secret_key = "")
    {
        ...
        $sandbox    = (bool) get_settings('paypal_sandbox_mode');
        $paypal_url = $sandbox ? 'https://api.sandbox.paypal.com/v1/' : 'https://api.paypal.com/v1/';

        // (١) رمز وصول
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL            => $paypal_url . 'oauth2/token',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_TIMEOUT        => 30,
            CURLOPT_POST           => true,
        ...
        // (٢) حالة الأمر
        $curl = curl_init($paypal_url . 'checkout/orders/' . rawurlencode($paymentID));
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_TIMEOUT        => 30,
            CURLOPT_HTTPHEADER     => array(
        ...
        $state = isset($order->state) ? strtolower($order->state)
               : (isset($order->status) ? strtolower($order->status) : '');

        if (in_array($state, array('approved', 'completed'), true)) {
            return true;
        }

        log_message('error', 'paypal_payment: حالة غير مقبولة (' . $state . ') للأمر ' . $paymentID);
        return false;
    }
    ...
    {
        $fail = function ($msg) {
            log_message('error', 'stripe_payment: ' . $msg);
        ...
        return array(
            ...
            'status_msg'     => 'حالة الدفع في سترايب: ' . ($status !== '' ? $status : 'غير معروفة'),
        );
    }
