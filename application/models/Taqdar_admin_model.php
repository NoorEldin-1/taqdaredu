...
class Taqdar_admin_model extends CI_Model
{
    ...
    public function spec($key = null)
    {
        $modules = array(
            ...
            /* TQ-FND-PACK — باقات حصص التأسيس.
               والوحدة الموصوفة تكفي: جدول وحقول وقائمة، بلا شاشة تكتب —
               وهو مبدأ `spec()` كله. والذي لا تكفيه هو سؤال «أتعمل؟»،
               وجوابه في شاشة «قسم التأسيس». */
            'foundation_packs' => array(
                'table'    => 'tq_foundation_packs',
                'title'    => 'باقات حصص التأسيس',
                'lead'     => 'عدد حصص بثمن واحد يدفع مرة، ويحجز به الطالب متى شاء — بدل أن يدفع عن كل حصة وحدها.',
                'icon'     => 'package',
                'ensure'   => 'taqdar_foundation_model',
                'order_by' => array('order' => 'ASC'),
                'note'     => 'الباقة رصيد لا مواعيد: يشتريها الطالب فيصير له عدد حصص يحجز بها في مسارها '
                            . 'متى فتح معلموه أوقاتا — ولا تحجز له شيئا سلفا. وثمنها يقارن تلقائيا بسعر الحصة '
                            . 'المفردة في المسار، فالتوفير يحسب ولا يكتب.',
                'fields'   => array(
                    'name_ar'         => array('label' => 'اسم الباقة', 'type' => 'text', 'required' => true, 'list' => true,
                                               'hint' => 'كما يقرؤه الطالب — «باقة ٦ حصص». والعدد في الاسم يقرأ أسرع من العدد في عمود.'),
                    'track_id'        => array('label' => 'المسار', 'type' => 'ref', 'ref' => 'foundation_tracks',
                                               'required' => true, 'list' => true,
                                               'hint' => 'الباقة لمسار واحد: رصيدها يحجز به في هذا المسار وحده. '
                                                       . 'وباقة تصلح لكل المسارات تجعل من اشتراها بسعر العربية يحجز بها في مسار أغلى.'),
                    'tagline'         => array('label' => 'السطر التعريفي', 'type' => 'text', 'list' => true,
                                               'hint' => 'سطر واحد تحت الاسم في البطاقة — «الأنسب لمن يبدأ من الصفر».'),

                    'sessions'        => array('label' => 'عدد الحصص', 'type' => 'number', 'required' => true, 'list' => true,
                                               'default' => 6, 'section' => 'الثمن والمدة',
                                               'hint' => 'كم حصة يحجزها صاحب الباقة. ويجمد وقت الشراء: رفعه غدا لا يزيد رصيد من اشترى أمس.'),
                    'price_halalas'   => array('label' => 'ثمن الباقة كاملة', 'type' => 'money', 'required' => true, 'list' => true,
                                               'hint' => 'يدفع مرة واحدة. والسعر المشطوب في البطاقة يحسب تلقائيا = عدد الحصص × سعر الحصة في المسار، '
                                                       . 'فاكتب هنا ما دون ذلك ليظهر التوفير.'),
                    'validity_days'   => array('label' => 'مدة صلاحية الرصيد (يوما)', 'type' => 'number', 'default' => 60, 'list' => true,
                                               'hint' => 'كم يوما يبقى الرصيد صالحا من يوم الدفع. و«صفر» يعني رصيدا لا ينتهي — '
                                                       . 'وهو ما تختاره إن لم ترد أن يحرق على أحد ما دفع ثمنه.'),
                    'teacher_percent' => array('label' => 'نصيب المعلم %', 'type' => 'percent',
                                               'mirror' => 'عمولة المنصة', 'placeholder' => 'نسبة المسار',
                                               'hint' => 'يقيد لمعلم كل حصة حين يعلن انتهاءها، من نصيب تلك الحصة من ثمن الباقة. '
                                                       . 'واتركه فارغا ليأخذ نسبة المسار. وانتبه: الباقة مخفضة، فالنسبة نفسها تعطي المعلم أقل.'),

                    'featured'        => array('label' => 'باقة مميزة', 'type' => 'bool', 'default' => 0,
                                               'section' => 'العرض والترتيب',
                                               'hint' => 'تبرز في صفحة المسار — «الأكثر طلبا».'),
                    'active'          => array('label' => 'متاحة', 'type' => 'bool', 'default' => 1, 'list' => true,
                                               'hint' => 'الموقوفة لا تعرض ولا تشترى — وأرصدة من اشتراها قبل إيقافها تبقى صالحة حتى تنتهي.'),
                    'order'           => array('label' => 'الترتيب', 'type' => 'number', 'default' => 0, 'list' => true),
                ),
            ),
    ...
    public function options($ref)
    {
        ...
        switch ($ref) {
            ...
            /* TQ-FND-PACK — مسارات التأسيس، مرجع الباقة الى مسارها.
               والمعطل يعرض بعلامته لا يحذف، بالعلة نفسها التي تعرض بها
               الباقة المعطلة: مسار عطل بعد ان ربطت به باقة يختفي من
               القائمة فيبدو الحقل فارغا وهو ليس فارغا، فيحفظ المسؤول
               تعديلا في حقل اخر ويمسح الربط بلا ان يقصد. */
            case 'foundation_tracks':
                try {
                    $this->load->model('taqdar_foundation_model', 'tq_fnd_opt');
                    foreach ($this->tq_fnd_opt->all() as $id => $t) {
                        $out[$id] = $t['name'] . ($t['active'] ? '' : ' (' . t('معطل') . ')');
                    }
