...
class User_model extends CI_Model
{
    ...
    public function add_user($is_instructor = false, $is_admin = false)
    {
        ...
        if ($validity == false) {
          ...
          //  $data['unique_identifier'] = 0;
            ...
            $data['password'] = tq_password_hash(html_escape($this->input->post('password')));
    ...
    public function add_shortcut_user($is_instructor = false)
    {
        ...
        if ($tq_email === '' || $tq_pass === '' || $tq_first === ''
            || !filter_var($tq_email, FILTER_VALIDATE_EMAIL)) {
            return json_encode(array(
                'status'  => 0,
                'message' => get_phrase('Please fill in the name, email and password.'),
            ));
        }

        $validity = $this->check_duplication('on_create', $tq_email);
        if ($validity == false) {
            $response['status'] = 0;
    ...
    public function unlock_screen_by_password($password = "")
    {
        ...
        return tq_password_authenticate($user, $password) ? 1 : 0;
    }
    ...
    public function new_device_login_tracker($user_id = "", $is_verified = '')
    {
        ...
        if(is_array($pre_sessions) && count($pre_sessions) > 0){
            ...
            foreach($live_rows as $live_row){
                $live_ids[] = $live_row['id'];
            }
            $pre_sessions = array_values(array_intersect($pre_sessions, $live_ids));
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row->role_id == 1) {
                ...
                if($this->session->userdata('url_history')){
                    $back = $this->session->userdata('url_history');
