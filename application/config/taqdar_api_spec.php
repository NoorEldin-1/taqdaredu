...
/* ---------------------------------------------------------------------
   TQ-SIGNUP — التسجيل والتأكيد. القواعد في `Taqdar_signup_model` نفسها
   التي يناديها نموذج الويب (`login/register`).
   ------------------------------------------------------------------- */

'/api/v1/auth/register/options' => array('get' => array(
    'tags' => array('Authentication'),
    'summary' => 'Sign-up form options',
    'description' => implode("\n", array(
        'Everything the sign-up form is built from: which gates are open, the active grades, the',
        '22 dial codes with their lengths and examples, the field limits, and whether a verification',
        'code will be sent and on which channels. Build the form from this, not from a copy in Dart —',
        'a grade disabled in the admin panel tomorrow must not stay in the app and then be refused.',
        '',
        '`gates[].available: false` on `teacher` means teacher applications are closed on this install;',
        'hide the option.',
    )),
    'security' => array(),
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                ...
                'grades' => array(array('id' => 7, 'name' => 'الصف الأول الابتدائي', 'name_en' => 'Grade 1')),
                'countries' => array(array('iso' => 'SA', 'dial_code' => '+966', 'name' => 'السعودية', 'flag' => '🇸🇦',
                                           'min_length' => 9, 'max_length' => 9, 'starts_with' => array('5'), 'example' => '512345678')),
                'default_country' => 'SA',
                'limits' => array(
                    'name' => array('min' => 2, 'max' => 40), 'email' => array('max' => 50),
                    'password' => array('min' => 8, 'max_bytes' => 72), 'age' => array('min' => 5, 'max' => 99),
                    'document' => array('types' => array('pdf', 'jpg', 'jpeg', 'png'), 'max_bytes' => 5242880),
                ),
                'verification' => array('required' => true, 'channels' => array('email', 'whatsapp')),
                'terms_url' => $TQ_API_BASE . '/terms', 'privacy_url' => $TQ_API_BASE . '/privacy',
            ),
            'message' => '', 'meta' => new stdClass(),
        )))),
        '429' => $r_429,
...
'/api/v1/auth/register' => array('post' => array(
    'tags' => array('Authentication'),
    'summary' => 'Create an account',
    'description' => implode("\n", array(
        'Creates a student, parent or teacher account with **the same rules as the website form** —',
        'both call `Taqdar_signup_model`, so the app never accepts what the site refuses.',
        '',
        'Send JSON for student and parent. A **teacher** must send `multipart/form-data` because the',
        'identity `document` (PDF · JPG · PNG, 5 MB) is required.',
        '',
        '#### What comes back depends on the gate',
        '',
        '| gate | account | `token` | next step |',
        '|---|---|---|---|',
        '| `student` · `parent` | open now (`status: active`) | a token pair — the user is signed in | show the verification banner; nothing is blocked |',
        '| `teacher` | closed until an admin approves | `null` | enter the code with `verification.ticket`, then wait for approval |',
        '',
        '`verification` says whether a code was sent, where (`sent_to` is masked), and which channels',
        'can resend it. It never blocks a student or parent: the code confirms the address, it is not a',
        'gate to the account (TQ-INSTANT, same as the website). When no mail or WhatsApp is configured',
        'on the install, `required` is `false` and the account simply opens.',
        '',
        '`verification.ticket` (`tqv_…`) is issued **only** for an account that cannot hold a token',
        '(the teacher). It lives 24 hours, is single-use, and opens nothing except `/auth/verify`.',
        '',
        '`otp_channel` is a wish, not an order: a channel the account cannot use falls back to the',
        'default. Students default to WhatsApp, others to e-mail.',
        '',
        '| `code` | status | meaning |',
        '|---|---|---|',
        '| `validation_failed` | 422 | `errors` names **every** bad field at once |',
        '| `email_taken` | 409 | send them to log in or reset their password |',
        '| `gate_closed` | 403 | teacher applications are closed on this install |',
        '| `rate_limited` | 429 | ten sign-ups per hour per IP |',
        '',
        'A file with a dangerous extension (`.exe`, `.php`, `.js`, …) is refused by a guard that runs',
        'before the API itself: the reply is `400` with the plain text `File type not allowed.`, not',
        'JSON. The app only ever sends PDF or an image, so it never meets it.',
    )),
    'security' => array(),
    'requestBody' => array('required' => true, 'content' => array(
        'application/json' => array(
            'schema' => array(
                'type' => 'object',
                'required' => array('gate', 'first_name', 'last_name', 'email', 'password', 'phone', 'accept_terms'),
                'properties' => array(
                    'gate'          => array('type' => 'string', 'enum' => array('student', 'parent', 'teacher'), 'default' => 'student'),
                    'first_name'    => array('type' => 'string', 'minLength' => 2, 'maxLength' => 40),
                    'last_name'     => array('type' => 'string', 'minLength' => 2, 'maxLength' => 40),
                    'email'         => array('type' => 'string', 'format' => 'email', 'maxLength' => 50),
                    'password'      => array('type' => 'string', 'format' => 'password', 'minLength' => 8, 'description' => 'At most 72 bytes (bcrypt).'),
                    'accept_terms'  => array('type' => 'boolean', 'description' => 'Must be true.'),
                    'phone'         => array('type' => 'string', 'example' => '0512345678', 'description' => 'National number in any common form; stored as +<dial><national>.'),
                    'phone_country' => array('type' => 'string', 'example' => 'SA', 'description' => 'ISO code from /auth/register/options. Default SA.'),
                    'age'           => array('type' => 'integer', 'minimum' => 5, 'maximum' => 99, 'description' => 'Student only, required.'),
                    'grade_id'      => array('type' => 'integer', 'description' => 'Student only, optional. Must be an active grade.'),
                    'national_id'   => array('type' => 'string', 'description' => 'Student only, optional. Ten digits; anything else is dropped, not refused.'),
                    'otp_channel'   => array('type' => 'string', 'enum' => array('email', 'whatsapp')),
                    'device_name'   => array('type' => 'string'), 'platform' => array('type' => 'string'),
                    'app_version'   => array('type' => 'string'), 'device_id' => array('type' => 'string'),
                ),
            ),
            'example' => array(
                'gate' => 'student', 'first_name' => 'سارة', 'last_name' => 'العتيبي',
                'email' => 'sara@example.com', 'password' => 'Secret1234', 'accept_terms' => true,
                'phone' => '0512345678', 'phone_country' => 'SA', 'age' => 12, 'grade_id' => 7,
                'otp_channel' => 'email', 'platform' => 'android', 'device_name' => 'Pixel 8',
            ),
        ),
        'multipart/form-data' => array('schema' => array(
            'type' => 'object',
            'description' => 'Teacher only. The same fields as JSON, plus the file.',
            'properties' => array(
                'document'     => array('type' => 'string', 'format' => 'binary', 'description' => 'PDF · JPG · PNG, up to 5 MB. Required for teachers.'),
                'message'      => array('type' => 'string', 'description' => 'A short introduction.'),
                'sample_url'   => array('type' => 'string', 'format' => 'uri', 'description' => 'A 10-minute sample lesson (YouTube, Drive…).'),
                'sample_note'  => array('type' => 'string'),
                'subject_hint' => array('type' => 'string'),
            ),
        )),
    )),
    'responses' => array(
        '201' => array('description' => 'Created. Student/parent are signed in; a teacher gets a ticket instead of a token.',
            'content' => array('application/json' => array('example' => array(
            'data' => array(
                'user' => array(
                    'id' => 512, 'first_name' => 'سارة', 'last_name' => 'العتيبي', 'name' => 'سارة العتيبي',
                    'email' => 'sara@example.com', 'phone' => '+966512345678',
                    'avatar_url' => $TQ_API_BASE . '/assets/taqdar/brand/avatar.svg',
                    'role' => 'student', 'status' => 'active', 'grade_id' => 7,
                    'created_at' => '2026-09-23T13:19:50+03:00', 'email_verified_at' => null,
                ),
                'token' => array(
                    'access_token' => 'tqa_9f3cAb7dQ1sK...', 'refresh_token' => 'tqr_1a8eZz0pLm4X...',
                    'token_type' => 'Bearer', 'expires_in' => 900, 'refresh_expires_in' => 2592000,
                ),
                'gate' => 'student', 'resumed' => false, 'application' => null,
                'verification' => array(
                    'required' => true, 'sent' => true, 'channel' => 'email', 'sent_to' => 'sa••••@example.com',
                    'retry_after' => 60, 'error' => null,
                    'channels' => array(
                        array('key' => 'email', 'label' => 'بريدك الإلكتروني', 'sent_to' => 'sa••••@example.com', 'guardian' => false),
                        array('key' => 'whatsapp', 'label' => 'واتساب', 'sent_to' => '+966••••••678', 'guardian' => false),
                    ),
                    'why' => null, 'ticket' => null,
                ),
            ),
            'message' => 'أنشئ حسابك ودخلت.', 'meta' => new stdClass(),
        )))),
        '400' => array('description' => 'A dangerous file extension, refused before the API runs. Plain text, not JSON.'),
        '403' => $err_ref('Teacher applications are closed on this install.',
            array('message' => 'التسجيل معلما متوقف حاليا. تواصل معنا للانضمام.', 'code' => 'gate_closed')),
        '409' => $err_ref('The e-mail already has an account.',
            array('message' => 'هذا البريد مسجل بالفعل — أدخل كلمة المرور لتدخل، أو استعدها إن نسيتها.',
                  'code' => 'email_taken', 'errors' => array('email' => array('هذا البريد مسجل بالفعل.')))),
        '422' => $err_ref('Every invalid field at once.',
            array('message' => 'راجع البيانات المدخلة.', 'code' => 'validation_failed',
                  'errors' => array('phone' => array('رقم الجوال في السعودية يكون 9 أرقام بعد رمز الدولة — مثل: 512345678.'),
                                    'age' => array('اكتب عمرا صحيحا بين 5 و99.')))),
        '429' => $r_429,
    ),
)),

'/api/v1/auth/verify' => array(
    'get' => array(
        'tags' => array('Authentication'),
        'summary' => 'Verification status',
        'description' => implode("\n", array(
            'Where verification stands: whether the account is verified, the channels a code can go to',
            '(masked), and the last code sent — `resend_in` seconds before another may be requested,',
            '`tries_left` on the current code.',
            '',
            'Identify with **either** the access token (`Authorization: Bearer`) **or** the verification',
            'ticket (`X-Verification-Ticket: tqv_…`) — never an e-mail in the request. An e-mail would let',
            'anyone who knows an address burn its codes and lock it out with five wrong guesses.',
        )),
        'security' => array(array('bearerAuth' => array()), array()),
        'parameters' => array(array('name' => 'X-Verification-Ticket', 'in' => 'header', 'required' => false,
                                    'schema' => array('type' => 'string'), 'description' => 'When there is no access token.')),
        'responses' => array(
            '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
                'data' => array(
                    'verified' => false, 'email_verified_at' => null,
                    'channels' => array(array('key' => 'email', 'label' => 'بريدك الإلكتروني', 'sent_to' => 'sa••••@example.com', 'guardian' => false)),
                    'default_channel' => 'email', 'why' => null,
                    'last_sent' => array('channel' => 'email', 'sent_to' => 'sa••••@example.com', 'delivered' => true,
                                         'expires_in' => 585, 'resend_in' => 45, 'tries_left' => 5, 'used' => false),
                ),
                'message' => '', 'meta' => new stdClass(),
            )))),
            '401' => $err_ref('No token and no ticket, or the ticket is spent or expired.',
                array('message' => 'تذكرة التأكيد غير صالحة أو انتهت. سجل دخولك لتصدر غيرها.', 'code' => 'ticket_invalid')),
            '429' => $r_429,
        ),
    ),
    'post' => array(
        ...
        'description' => implode("\n", array(
            'Six digits, valid ten minutes, five tries, single use — the same `Taqdar_otp_model` the',
            'website uses. A correct code stamps `email_verified_at`.',
            '',
            '- Student or parent: the account is (re)opened. **If they came with a ticket** (a stuck',
            '  account from the login rescue) they are signed in and `token` carries a pair.',
            '- Teacher: the contact is confirmed, but the account **stays closed** until an admin approves',
            '  the application — `token` is `null` and `application.status` is `pending`.',
            '',
            'The ticket is spent on success.',
            '',
            '| `code` | status | what to do |',
            '|---|---|---|',
            '| `invalid_code` | 422 | wrong digits; the message says how many tries are left |',
            '| `code_expired` · `code_used` | 410 | ask for a new code |',
            '| `code_not_requested` | 409 | nothing was sent yet — call `/auth/verify/resend` |',
            '| `too_many_attempts` | 429 | five wrong tries on this code — ask for a new one |',
            '| `already_verified` | 409 | nothing to do |',
        )),
        'security' => array(array('bearerAuth' => array()), array()),
        'requestBody' => array('required' => true, 'content' => array('application/json' => array(
            'schema' => array('type' => 'object', 'required' => array('code'), 'properties' => array(
                'code' => array('type' => 'string', 'pattern' => '^[0-9]{6}$', 'example' => '483920'),
                'verification_ticket' => array('type' => 'string', 'description' => 'Or send it as the X-Verification-Ticket header.'),
            )),
            'example' => array('code' => '483920'),
        ))),
        'responses' => array(
            '200' => array('description' => 'Verified.', 'content' => array('application/json' => array('example' => array(
                'data' => array(
                    'verified' => true,
                    'user' => array('id' => 512, 'name' => 'سارة العتيبي', 'role' => 'student', 'status' => 'active',
                                    'email_verified_at' => '2026-09-23T13:20:47+03:00'),
                    'token' => null, 'application' => null,
                ),
                'message' => 'أكدنا حسابك.', 'meta' => new stdClass(),
            )))),
            '401' => $r_401,
            '409' => $err_ref('Already verified, or no code was requested yet.',
                array('message' => 'حسابك مؤكد بالفعل.', 'code' => 'already_verified')),
            '410' => $err_ref('The code expired or was already used.',
                array('message' => 'انتهت صلاحية الرمز. اطلب رمزا جديدا.', 'code' => 'code_expired')),
            '422' => $err_ref('Wrong code.',
                array('message' => 'الرمز غير صحيح. بقيت لك 4 محاولة.', 'code' => 'invalid_code',
                      'errors' => array('code' => array('الرمز غير صحيح. بقيت لك 4 محاولة.')))),
            '429' => $r_429,
        ),
    ),
),

'/api/v1/auth/verify/resend' => array('post' => array(
    'tags' => array('Authentication'),
    'summary' => 'Send a new verification code',
    'description' => implode("\n", array(
        'Sends a fresh code (the previous one dies). `channel` is optional — without it the last',
        'channel used, else the default. The destination is always taken **from the account**, never',
        'from the request, so this cannot be used to send messages to an arbitrary number.',
        '',
        'Throttled by the OTP layer itself: 60 seconds between sends (`resend_too_soon` with',
        '`Retry-After`) and five per hour.',
    )),
    'security' => array(array('bearerAuth' => array()), array()),
    'requestBody' => array('required' => false, 'content' => array('application/json' => array(
        'schema' => array('type' => 'object', 'properties' => array(
            'channel' => array('type' => 'string', 'enum' => array('email', 'whatsapp')),
            'verification_ticket' => array('type' => 'string'),
        )),
        'example' => array('channel' => 'whatsapp'),
    ))),
    'responses' => array(
        '200' => array('description' => 'Sent.', 'content' => array('application/json' => array('example' => array(
            'data' => array('channel' => 'whatsapp', 'sent_to' => '+966••••••678', 'retry_after' => 60, 'expires_in' => 600),
            'message' => 'أرسلنا رمزا جديدا.', 'meta' => new stdClass(),
        )))),
        '401' => $r_401,
        '409' => $err_ref('Already verified.', array('message' => 'حسابك مؤكد بالفعل.', 'code' => 'already_verified')),
        '422' => $err_ref('A channel this account cannot use.',
            array('message' => 'راجع البيانات المدخلة.', 'code' => 'validation_failed',
                  'errors' => array('channel' => array('القناة غير متاحة. المتاح: email · whatsapp')))),
        '429' => $err_ref('Less than 60 seconds since the last code, or five in the last hour. Honour `Retry-After`.',
            array('message' => 'أمهل قليلا قبل طلب رمز جديد.', 'code' => 'resend_too_soon')),
        '502' => $err_ref('The mail or WhatsApp provider refused. Try the other channel.',
            array('message' => 'تعذر إرسال الرمز بواتساب. جرب البريد الإلكتروني.', 'code' => 'send_failed')),
        '503' => $err_ref('No channel is configured on this install.',
            array('message' => 'لا قناة متاحة لإرسال الرمز الآن.', 'code' => 'no_channel')),
    ),
)),
...
'/api/v1/auth/login' => array('post' => array(
    ...
    'description' => implode("\n", array(
        ...
        '| `email_not_verified` | Signed up, never confirmed | `errors.details.verification_ticket` is included: call `/auth/verify/resend`, then `/auth/verify`, with it |',
        '| `account_disabled` | Confirmed once, then suspended by an admin (403) | Show the message; nothing to retry |',
        ...
        '',
        'New accounts come from `POST /auth/register`. A `verification_ticket` is issued only after the',
        'password was proven correct, so it reveals nothing to someone who does not hold it.',
    )),
    'security' => array(),
    'requestBody' => array('required' => true, 'content' => array('application/json' => array(
        'schema' => array(
    ...
    'responses' => array(
        ...
        '403' => $err_ref('Credentials were right; the account is not usable yet. See the table above.',
            array('message' => 'بريدك لم يؤكد بعد. اطلب رمز التأكيد واكتبه ليفتح حسابك.', 'code' => 'email_not_verified',
                  'errors' => array('details' => array('verification_ticket' => 'tqv_md9_-sVKLqFaauZis...')))),
        '422' => $r_422,
        '429' => $r_429,
    ),
)),

'/api/v1/auth/social' => array('post' => array(
    'tags' => array('Authentication'),
    'summary' => 'Log in with Google or Apple',
    'description' => implode("\n", array(
        'Exchange a provider `id_token` for the same token pair `/auth/login` returns.',
        '',
        'Sign in natively - `google_sign_in` on Android/iOS, `sign_in_with_apple` on iOS - and',
        'post the resulting `id_token` here. **Do not open the website OAuth flow in a WebView**:',
        'that is the browser journey, both providers discourage it, and it drops the user out of',
        'the app and back in a different state.',
        '',
        'The server verifies the token signature against the provider published keys, plus issuer,',
        'audience and expiry. An `id_token` minted for a different app is rejected - so the iOS and',
        'Android client ids must be filled in on the admin screen, not just the web one.',
        '',
        '#### First sign-in creates the account',
        '',
        'There is no separate sign-up call. An unknown provider account with a verified e-mail gets',
        'a Taqdar account created on the spot (`created: true`), as a **student** unless you send',
        '`gate: "parent"`. A provider account whose verified e-mail already belongs to a Taqdar',
        'account is linked to it, so a user who signed up with a password keeps their subscription.',
        '',
        'Teachers are never created this way - a teacher application needs a phone, a document and a',
        'sample lesson. An existing teacher can still sign in with Google.',
        '',
        '#### Apple sends the name once, ever',
        '',
        'Apple returns `givenName`/`familyName` **only on the very first authorization on that',
        'device**, and never again. Forward it as `given_name`/`family_name` on that first call or',
        'the account keeps a name derived from the e-mail forever.',
        '',
        '#### `missing_profile`',
        '',
        'Neither provider gives a phone number or an age. The response lists what is still missing',
        '(`["phone","age"]`) - show a short completion sheet. It is **not** a block: the account is',
        'open and every other endpoint works.',
        '',
        '| `code` | What happened | What the app should do |',
        '|---|---|---|',
        '| `invalid_social_token` | Signature, audience or expiry failed (401) | Ask the provider for a fresh token and retry once |',
        '| `provider_not_configured` | Keys not set on this install (503) | Hide the button |',
        '| `email_not_shared` | Apple user hid the e-mail entirely (422) | Ask them to allow e-mail sharing |',
        '| `email_not_verified_by_provider` | Provider gave an e-mail it has not verified (422) | Tell them to verify it with the provider first |',
        '| `teacher_pending_approval` | Teacher awaiting review (403) | Show the message |',
        '| `admin_not_allowed` | Admin account (403) | Admins use the web panel only |',
    )),
    'security' => array(),
    'requestBody' => array('required' => true, 'content' => array('application/json' => array(
        'schema' => array(
            'type' => 'object',
            'required' => array('provider', 'id_token'),
            'properties' => array(
                'provider'    => array('type' => 'string', 'enum' => array('google', 'apple'), 'example' => 'google'),
                'id_token'    => array('type' => 'string', 'description' => 'The provider-signed JWT from the native sign-in SDK.'),
                'gate'        => array('type' => 'string', 'enum' => array('student', 'parent'), 'default' => 'student',
                                       'description' => 'Only used when the account is created.'),
                'given_name'  => array('type' => 'string', 'maxLength' => 40, 'description' => 'Apple only, first authorization only.'),
                'family_name' => array('type' => 'string', 'maxLength' => 40, 'description' => 'Apple only, first authorization only.'),
                'device_name' => array('type' => 'string', 'maxLength' => 120, 'example' => 'iPhone 15'),
                'device_id'   => array('type' => 'string', 'maxLength' => 120, 'example' => 'a41f9c2e-77b0'),
                'platform'    => array('type' => 'string', 'maxLength' => 32, 'example' => 'ios'),
                'app_version' => array('type' => 'string', 'maxLength' => 32, 'example' => '1.0.0'),
            ),
        ),
        'example' => array(
            'provider' => 'apple', 'id_token' => 'eyJraWQiOiJXNldjT0tCIiwiYWxnIjoiUlMyNTYifQ...',
            'given_name' => 'سارة', 'family_name' => 'العتيبي',
            'platform' => 'ios', 'device_name' => 'iPhone 15', 'app_version' => '1.0.0',
        ),
    ))),
    'responses' => array(
        '200' => array('description' => 'Signed in. `created` says whether the account was made just now.',
            'content' => array('application/json' => array('example' => array(
            'data' => array(
                'token' => array(
                    'access_token' => 'tqa_9f3cAb7dQ1sK...', 'refresh_token' => 'tqr_1a8eZz0pLm4X...',
                ...
                'user' => array(
                    'id' => 512, 'first_name' => 'سارة', 'last_name' => 'العتيبي',
                    'name' => 'سارة العتيبي', 'email' => 'sara@example.com',
                    'phone' => '', 'avatar_url' => null,
                    'role' => 'student', 'status' => 'active', 'grade_id' => null,
                    'created_at' => '2026-09-12T10:04:00+03:00',
                ),
                'created'         => true,
                'missing_profile' => array('phone', 'age'),
            ),
            'message' => 'أنشئ حسابك. أهلا بك.', 'meta' => new stdClass(),
        )))),
        '401' => $err_ref('The provider token did not verify - bad signature, wrong audience, or expired.',
            array('message' => 'هذه الهوية صادرة لتطبيق آخر.', 'code' => 'invalid_social_token')),
        '403' => $err_ref('The identity is valid but the account cannot be used from the app.',
            array('message' => 'حسابات الإدارة تدخل من لوحة الويب لا من التطبيق.', 'code' => 'admin_not_allowed')),
        '422' => $r_422,
        '429' => $r_429,
        '503' => $err_ref('This provider has no keys configured on this install.',
            array('message' => 'هذه الطريقة غير متاحة حاليا.', 'code' => 'provider_not_configured')),
    ),
)),
...
'/api/v1/auth/me' => array('get' => array(
    ...
    'description' => implode("\n", array(
        'The account behind the token. Cheapest way to validate a stored session on app start — one query,',
        'and it tells you the role before you route to a dashboard.',
        '',
        '**A role block comes with it.** A `teacher` token also returns `teacher` — the teaching scope',
        '(subject and grade, and the full lists when there is more than one) plus course and student counts.',
        'A `parent` token returns `parent` — how many children are linked, and how many requests are still',
        'waiting on a child\'s approval. Without them the teacher settings header printed an empty',
        '"teaching scope" line on every live account, and a parent of three read "linked children · 0";',
        'nothing was wrong, the answer simply was not in the payload.',
        '',
        'A student token carries `grade_id` in the user body as before, and no extra block.',
        '',
        '```json',
        '{ "data": { …, "role": "teacher",',
        '    "teacher": {',
        '      "subject":  { "id": 3, "name": "الرياضيات" },',
        '      "grade_id": 20, "grade_name": "الصف الرابع الابتدائي",',
        '      "courses": 2, "students": 24 } } }',
        '',
        '{ "data": { …, "role": "parent",',
        '    "parent": { "children": 3, "pending_links": 1 } } }',
        '```',
    )),
    'security' => $auth,
    'responses' => array(
...
'/api/v1/auth/sessions' => array('get' => array(
    ...
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(array(
                'id' => '7b1c9f4e2a8d4f6b9c0e1a2b3c4d5e6f',
                'device_name' => 'Galaxy S23', 'platform' => 'android', 'app_version' => '1.0.0',
...
'/api/v1/auth/sessions/{id}' => array('delete' => array(
    'tags' => array('Authentication'),
    'summary' => 'Sign out one device',
    'description' => implode("\n", array(
        'Revokes the token family of a single device. `id` is the `id` field of a row from',
        '`GET /auth/sessions` — it is stable across refreshes, so it is the device\'s name for life.',
        '',
        'Without it the devices screen had one button for every row — `logout-all` — so "I lost my phone"',
        'meant signing the laptop and the tablet out too. That is the state the screen was written for.',
        '',
        'Ownership is a condition of the `UPDATE`, not a check before it: a guessed id signs nobody out.',
        'Revoking the current device is allowed — that is `logout` by id, and it is what the row means.',
    )),
    'security' => $auth,
    'parameters' => array(array('name' => 'id', 'in' => 'path', 'required' => true,
                                'schema' => array('type' => 'string'),
                                'description' => 'The `id` of a row from GET /auth/sessions.')),
    'responses' => array(
        '200' => array('description' => 'Device signed out.', 'content' => array('application/json' => array('example' => array(
            'data' => array('id' => '7b1c9f4e2a8d4f6b9c0e1a2b3c4d5e6f', 'was_current' => false),
            'message' => 'أخرجت ذلك الجهاز.', 'meta' => new stdClass(),
        )))),
        '401' => $r_401,
        '404' => $err_ref('No live session with that id on this account — already signed out, or not yours.',
            array('message' => 'لا جهاز بهذا المعرف في حسابك.', 'code' => 'not_found')),
        '429' => $r_429,
    ),
)),
...
'/api/v1/student/bundle' => array('get' => array(
    'tags' => array('Subscription'),
    'summary' => 'What my plan opens',
    'description' => implode("\n", array(
        'The courses, books and materials the student\'s current subscription actually opens.',
        '',
        'This answers a different question from `/student/subscription`: that one says *until when*,',
        'this one says *what*. It was the one section of the student portal with a full web screen',
        '(`student/bundle`) and no endpoint at all — so it could not be built in the app: there was',
        'nothing to read.',
        '',
        'Books are those the subscription **grants** (`granted_book_ids()` — the same source the file',
        'guard asks), not every free published book; `/student/library` is the wider shelf.',
        '',
        '**No plan is not a 404.** Empty lists and `plan: null`: a student can own a single course or a',
        'book with no plan at all, and an error would make a working screen look broken. `meta.note`',
        'carries the Arabic sentence for the empty state — written server-side, like every other',
        'user-facing line.',
    )),
    'security' => $auth,
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                'plan' => array('id' => 4, 'code' => 'full-g4', 'name' => 'باقة الصف الرابع',
                                'status' => 'active', 'starts_at' => '2026-08-06T00:00:00+03:00',
                                'ends_at' => '2027-08-06T00:00:00+03:00',
                                'cover_url' => $TQ_API_BASE . '/uploads/plans/g4.webp'),
                'courses' => array(array('id' => 109, 'title' => 'الرياضيات — الفصل الأول',
                                         'subject' => 'الرياضيات', 'teacher' => 'أحمد',
                                         'thumbnail' => $TQ_API_BASE . '/uploads/thumbnails/course_thumbnails/a1.jpg',
                                         'progress' => 40, 'next_lesson_id' => 812)),
                'books' => array(array('id' => 14, 'title' => 'كتاب الطالب', 'subject' => 'الرياضيات',
                                       'web_url' => $TQ_API_BASE . '/book/math-g4',
                                       'file_url' => $TQ_API_BASE . '/api/v1/student/books/14/file')),
                'materials' => array(array('id' => 7, 'title' => 'ملخص الوحدة الأولى',
                                           'course' => 'الرياضيات — الفصل الأول', 'kind' => 'pdf',
                                           'file_url' => $TQ_API_BASE . '/uploads/lesson_files/u1.pdf')),
            ),
            'message' => '',
            'meta' => array('counts' => array('courses' => 1, 'books' => 1, 'materials' => 1),
                            'note' => 'هذا ما تفتحه باقتك اليوم.'),
        )))),
        '401' => $r_401, '403' => $r_403, '429' => $r_429,
    ),
)),

'/api/v1/student/plan' => array('get' => array(
    'tags' => array('Progress'),
    'summary' => 'One day of the plan',
    'description' => implode("\n", array(
        'The plan for a single day. `date` is `YYYY-MM-DD`; omit it for today.',
        '',
        '"My plan" in the app was assembled from `/student/home`, and that knows **today** only — so',
        'every other day came back empty, with nothing saying why.',
        '',
        'A past day has no plan to invent; it has what actually happened, and that is recorded. So today',
        'returns `next_step` in exactly the shape `/student/home` returns it (the same `next_step()`),',
        'and every day returns its events from the same source `/student/calendar` draws.',
        '',
        '**An empty day is an empty list, not a 404** — the day exists, and the emptiness is the answer.',
        'An unparseable date is a 422: that is a bad request, not an empty day.',
    )),
    'security' => $auth,
    'parameters' => array(array('name' => 'date', 'in' => 'query', 'required' => false,
                                'schema' => array('type' => 'string', 'format' => 'date'),
                                'description' => 'YYYY-MM-DD. Defaults to today.')),
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                'date' => '2026-09-08', 'is_today' => false, 'is_past' => true,
                'next_step' => null, 'goal' => null,
                'items' => array(array(
                    'at' => '2026-09-08T17:20:00+03:00', 'category' => 'exams',
                    'category_label' => 'اختبارات', 'icon' => 'check-badge',
                    'title' => 'سلمت: اختبار الوحدة الأولى', 'subtitle' => 'الرياضيات — الفصل الأول',
                    'web_url' => $TQ_API_BASE . '/student/lesson/109/812',
                )),
            ),
            'message' => '', 'meta' => array('total' => 1, 'note' => ''),
        )))),
        '401' => $r_401, '403' => $r_403, '422' => $r_422, '429' => $r_429,
    ),
)),
    ...
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                ...
                /* يخفي التطبيق بند «التأسيس» بالقاعدة التي يخفيه بها الموقع. */
                'features' => array('foundation' => true),
            ),
...
'/api/v1/student/courses/{id}/quiz/start' => array('post' => array(
    'tags' => array('Assessment'),
    'summary' => 'Start a course or station exam',
    'description' => implode("\n", array(
        'Opens (or resumes) an attempt on an assessment anchored to a **course** or one of its',
        '**milestones** — not to a single lesson.',
        '',
        'There was only one door into assessments before this: the lesson quiz. The app\'s "station exam"',
        'screen had no endpoint, so it shipped questions in its own asset files and marked them against',
        'a key it invented. A score computed on the client is not a score: it never reaches `attempts`,',
        'it moves no `skill_state`, no teacher or parent ever sees it, and whoever opens the bundle reads',
        'the answers.',
        '',
        '**The second call is unchanged**: submit to `POST /student/quiz/attempts/{id}/submit` and review',
        'at `GET /student/quiz/attempts/{id}`, exactly as for a lesson quiz. Only the anchor differs —',
        'the same principle that kept the lesson quiz from becoming a fourth quiz system.',
        '',
        '#### Which assessment',
        '',
        'Send `assessment_id` to name one. Otherwise the course-level exam is used; if there is none and',
        'the course has exactly one station exam, that one is used. More than one with no choice made',
        'returns `409 ambiguous_assessment` with the list in `errors.details.assessments` — the screen',
        'shows them, the server does not guess.',
        '',
        'Entitlement is checked against the course. There is no mastery lock here: the lock orders',
        '*lessons inside a course*, and a course exam is not a lesson in that order.',
        '',
        '**No answer key in the response**, here as everywhere: marking happens on the server.',
    )),
    'security' => $auth,
    'parameters' => array(array('name' => 'id', 'in' => 'path', 'required' => true,
                                'schema' => array('type' => 'integer'))),
    'requestBody' => array('required' => false, 'content' => array('application/json' => array(
        'schema' => array('type' => 'object', 'properties' => array(
            'assessment_id' => array('type' => 'integer',
                'description' => 'Optional. Names one of the course\'s assessments.'),
        )),
        'example' => array('assessment_id' => 31),
    ))),
    'responses' => array(
        '200' => array('description' => 'Attempt open.', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                'attempt_id' => 91, 'attempt_no' => 1, 'assessment_id' => 31,
                'assessment_type' => 'exam', 'course_id' => 109,
                'course_title' => 'الرياضيات — الفصل الأول',
                'milestone_id' => null, 'milestone_title' => null, 'lesson_id' => null,
                'pass_mark' => 6, 'time_limit_sec' => 600,
                'questions' => array(array('id' => 7781, 'title' => 'كم يساوي ٥ × ٥؟',
                                           'type' => 'radio', 'options' => array('٢٠', '٢٥', '٣٠'))),
                'submit_url' => $TQ_API_BASE . '/api/v1/student/quiz/attempts/91/submit',
            ),
            'message' => '', 'meta' => new stdClass(),
        )))),
        '401' => $r_401,
        '403' => $err_ref('The course is not part of this student\'s entitlement.',
            array('message' => 'هذا المحتوى غير متاح ضمن اشتراكك', 'code' => 'not_entitled')),
        '404' => $err_ref('No such course, or the assessment has no questions yet.',
            array('message' => 'هذا الاختبار لا أسئلة فيه بعد', 'code' => 'no_questions')),
        '409' => $err_ref('More than one assessment and none named. The list is in `errors.details.assessments`.',
            array('message' => 'لهذا المقرر أكثر من اختبار — حدد أيها تريد', 'code' => 'ambiguous_assessment')),
        '429' => $r_429,
    ),
)),
...
'/api/v1/student/mistakes/drill' => array('get' => array(
    'tags' => array('Practice'),
    'summary' => 'Mistake-notebook drill — questions',
    'description' => implode("\n", array(
        'Questions from the mistake notebook with their options, for a focused drill. The same model call',
        'the website drill uses, so a question that left the notebook on the site never returns here.',
        '',
        'Each row is the `/student/reviews` row plus `wrong_count`. **No `correct_answers`** — the server',
        'judges the answer. `course_id` filters on the server **before** `limit` is applied, and',
        '`meta.total` is how many questions the (filtered) notebook holds, so the screen can say',
        '"10 of 37".',
    )),
    'security' => $auth,
    'parameters' => array(
        array('name' => 'limit', 'in' => 'query', 'schema' => array('type' => 'integer', 'minimum' => 1, 'maximum' => 50, 'default' => 10)),
        array('name' => 'course_id', 'in' => 'query', 'schema' => array('type' => 'integer')),
    ),
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(array(
                'question_id' => 217, 'title' => 'أي الكسور يكافئ 2/4؟', 'type' => 'radio',
                'options' => array('1/2', '2/3', '3/4', '1/4'),
                'objective' => array('id' => 254, 'text' => 'أن يمثل الطالب الكسر بالرسم ويقرأه', 'at_second' => 45),
                'lesson' => array('id' => 88, 'title' => 'الكسور المتكافئة'),
                'course' => array('id' => 12, 'title' => 'الرياضيات — الصف الرابع'),
                'due_at' => null, 'interval_days' => 0, 'lapses' => 0, 'wrong_count' => 3,
            )),
            'message' => '', 'meta' => array('count' => 1, 'total' => 37),
        )))),
        '401' => $r_401, '403' => $r_403, '429' => $r_429,
    ),
)),

'/api/v1/student/mistakes/drill/answer' => array('post' => array(
    'tags' => array('Practice'),
    'summary' => 'Mistake-notebook drill — answer',
    'description' => implode("\n", array(
        'The server decides correctness from `given`; `correct` is never accepted from the client.',
        'The condition is that the student **got this question wrong before** — not that it is in the',
        'review queue. A scheduled question goes through the spaced-repetition engine (so `interval_days`',
        'and `due_at` come back); an unscheduled one only moves the skill state.',
        '',
        '`cleared: true` means the question left the notebook (right after its last wrong answer).',
    )),
    'security' => $auth,
    'requestBody' => array('required' => true, 'content' => array('application/json' => array(
        'schema' => array('type' => 'object', 'required' => array('question_id', 'given'), 'properties' => array(
            'question_id' => array('type' => 'integer'),
            'given' => array('description' => 'The chosen option text, or a list for multi-select.'),
        )),
        'example' => array('question_id' => 217, 'given' => '1/2'),
    ))),
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array('question_id' => 217, 'correct' => true, 'scheduled' => true, 'cleared' => true,
                            'still_wrong_count' => 3, 'remaining_due' => 2, 'interval_days' => 3,
                            'due_at' => '2026-09-26T13:00:00+03:00'),
            'message' => 'أحسنت — خرج هذا السؤال من دفترك.', 'meta' => new stdClass(),
        )))),
        '401' => $r_401,
        '403' => $err_ref('The question is not in this student\'s mistakes.',
            array('message' => 'هذا السؤال ليس في دفتر أخطائك.', 'code' => 'not_entitled')),
        '404' => $r_404, '422' => $r_422, '429' => $r_429,
    ),
)),

'/api/v1/student/lessons/{id}/transcript' => array('get' => array(
    'tags' => array('Lesson'),
    'summary' => 'Lesson transcript',
    'description' => implode("\n", array(
        'The transcript cue by cue with its second — tap a cue to seek the player.',
        '',
        'Guarded like the lesson itself: a locked lesson is `403 mastery_locked` (the transcript *is*',
        'the lesson), and a non-free lesson outside the subscription is `403 not_entitled`, the same',
        'check `/student/media` makes. A lesson with no transcript returns `cues: []`, not 404.',
    )),
    'security' => $auth,
    'parameters' => array(array('name' => 'id', 'in' => 'path', 'required' => true, 'schema' => array('type' => 'integer'))),
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array('lesson_id' => 91, 'cues' => array(
                array('at_second' => 0, 'at_label' => '00:00', 'text' => 'مرحبا بكم في درس الكسور.'),
                array('at_second' => 75, 'at_label' => '01:15', 'text' => 'الكسر جزء من كل.'),
            )),
            'message' => '', 'meta' => array('count' => 2),
        )))),
        '401' => $r_401,
        '403' => $err_ref('Locked by mastery, or outside the subscription.',
            array('message' => 'أكمل مراجعة الدرس السابق أولا.', 'code' => 'mastery_locked')),
        '404' => $r_404, '429' => $r_429,
    ),
)),
    ...
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(array(
                ...
                'score' => 9, 'percent' => 90, 'issued_at' => '2026-08-20T09:12:00+03:00',
    ...
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(array(
                ...
                'url' => $TQ_API_BASE . '/api/v1/student/materials/file/77/file',
                'bytes' => 184320, 'locked' => false, 'lock_hint' => '',
                'added_at' => '2026-08-10T10:00:00+03:00',
...
'/api/v1/parent/home' => array('get' => array(
    'tags' => array('Parent'),
    'summary' => 'Guardian dashboard',
    'description' => implode("\n", array(
        'The one opening call for the guardian portal: every child with the three simplified measures,',
        'the inbox badges, and any invoice still waiting to be paid.',
        '',
        'It was the only one of the three portals without it. `/student/home` and `/teacher/home` both',
        'carry `inbox`, which is where those screens read their badges from; the guardian screen had to',
        'call `/parent/messages` and `/parent/notifications` on every open — and `children` and `pay`',
        'besides, so five round trips on a phone before one line appeared.',
        '',
        'The measures come from the same model call `/parent/children/{id}` uses, so a parent never reads',
        'two numbers for one child on two adjacent screens. A `pending` link is not measured — its',
        'measures are `null` and `link_status` says why; a zero there would read as "your child is not',
        'studying" about a child who is not linked yet.',
        '',
        '**Privacy (TQ-LINK-ENUM):** `name` and `avatar_url` are `null` for a child who never consented;',
        'show the `email` the parent typed instead. Same rule as `/parent/children`.',
    )),
    'security' => $auth,
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                'user' => array('id' => 501, 'name' => 'ولي الاختبار',
                                'avatar_url' => $TQ_API_BASE . '/assets/taqdar/brand/avatar.svg'),
                'children' => array(
                    array(
                        'student_id' => 287, 'name' => 'طالب الاختبار',
                        'avatar_url' => $TQ_API_BASE . '/assets/taqdar/brand/avatar.svg',
                        'email' => 'student.test@taqdaredu.com',
                        'link_status' => 'active', 'commitment' => 72, 'understanding' => 65, 'trend' => 'up',
                    ),
                    /* لم يوافق قط: لا اسم ولا صورة، ويعرف بالبريد الذي كتبه أبوه. */
                    array(
                        'student_id' => 397, 'name' => null, 'avatar_url' => null,
                        'email' => 'noura@example.com',
                        'link_status' => 'pending', 'commitment' => null, 'understanding' => null, 'trend' => null,
                    ),
                ),
                'inbox' => array('messages' => 2, 'notifications' => 5),
                'due_invoices' => array(array(
                    'invoice_id' => 58, 'invoice_no' => 'TQ-2026-00006', 'student_id' => 287,
                    'student_name' => 'طالب الاختبار',
                    'amount' => array('amount' => 39900, 'decimal' => '399.00', 'currency' => 'SAR',
                                      'formatted' => '399.00 ر.س'),
                    'status' => 'unpaid', 'issued_at' => '2026-09-01T13:39:52+03:00',
                )),
            ),
            'message' => '', 'meta' => array('children_total' => 1, 'due_total' => 1),
        )))),
        '401' => $r_401, '403' => $r_403, '429' => $r_429,
    ),
)),

'/api/v1/parent/invoices/{id}/pay' => array('post' => array(
    'tags' => array('Parent'),
    'summary' => 'Pay a child\'s invoice by card',
    'description' => implode("\n", array(
        'Starts a card payment for an invoice issued in a child\'s name and returns a `payment_url`.',
        '',
        '**The invoice is always in the child\'s name** — they own the subscription and the content. So',
        '`POST /student/invoices/{id}/pay` is guarded by `require_student()` and refuses the parent, and',
        'a guardian who created a subscription by bank transfer and then wanted to pay by card had no',
        'route to it at all: the screen showed the invoice due and said to transfer it. The child is a',
        'minor with no card. The door was shut on both sides.',
        '',
        'The engine is unchanged: the gateway call already accepts a guardian linked by an **active**',
        'link — the same guard the rest of the portal reads by. What was missing was a door, not an',
        'engine. Everything after it — the return, the webhook, the reconciliation job — behaves exactly',
        'as it does for the student.',
        '',
        'The response is the same shape as the student endpoint\'s, so one client branch serves both.',
    )),
    'security' => $auth,
    'parameters' => array(array('name' => 'id', 'in' => 'path', 'required' => true,
                                'schema' => array('type' => 'integer'))),
    'responses' => array(
        ...
        '401' => $r_401, '403' => $r_403,
        '404' => $err_ref('No invoice with that id on any of your children. Missing and not-yours are deliberately not distinguished.',
            array('message' => 'لا فاتورة بهذا الرقم على أبنائك.', 'code' => 'not_found')),
        '409' => $err_ref('Invoice already paid or refunded.',
            array('message' => 'هذه الفاتورة ليست مستحقة السداد.', 'code' => 'invoice_not_payable')),
        '502' => $err_ref('The gateway refused to open a charge. The invoice still stands and can be paid by transfer.',
            array('message' => 'تعذر بدء الدفع. وفاتورتك صدرت، فيمكنك تحويل قيمتها بنكيا.', 'code' => 'payment_start_failed')),
...
'/api/v1/parent/children' => array('get' => array(
    ...
    'description' => implode("\n", array(
        ...
        '',
        '**Privacy (TQ-LINK-ENUM).** `name` and `avatar_url` are `null` until the child has consented at',
        'least once (active link, or `consent`/`previous_consent` on record) — the same rule the website',
        'settings screen uses. Show the `email` instead. `student_id` stays.',
        '',
        '`closed` is set on `revoked` rows: `reason` is `rejected` (the child declined the request) or',
        '`revoked` (a live link was undone); `by` is `student`, `parent` or `admin`.',
        '',
        '`plan_is_default: true` means no weekly plan was chosen and `plan_days` is the fallback.',
    )),
    'security' => $auth,
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                array(
                    ...
                    'avatar_url' => $TQ_API_BASE . '/assets/taqdar/brand/avatar.svg',
                    'status' => 'active', 'consent_at' => '2026-08-18T00:18:00+03:00',
                    'plan_days' => 4, 'plan_is_default' => false, 'closed' => null,
                ),
                array(
                    'link_id' => 182, 'student_id' => 397, 'name' => null, 'email' => 'noura@example.com',
                    'avatar_url' => null, 'status' => 'pending', 'consent_at' => null,
                    'plan_days' => 5, 'plan_is_default' => true, 'closed' => null,
                ),
                array(
                    'link_id' => 183, 'student_id' => 398, 'name' => null, 'email' => 'fahad@example.com',
                    'avatar_url' => null, 'status' => 'revoked', 'consent_at' => null,
                    'plan_days' => 5, 'plan_is_default' => true,
                    'closed' => array('reason' => 'rejected', 'by' => 'student', 'at' => '2026-07-28T21:32:18+03:00'),
                ),
            ),
...
'/api/v1/parent/children/{id}' => array('get' => array(
    ...
    'description' => implode("\n", array(
        ...
        '',
        '`quiz_results` are the rows the website child page shows: the **latest** attempt per lesson quiz',
        '(the question is "where is the child now?"), `total` being the number of questions. No single',
        'answer is exposed. `subjects[].mastery.percent` is `null` when no objective is open yet.',
    )),
    'security' => $auth,
    'parameters' => array(array('name' => 'id', 'in' => 'path', 'required' => true,
        ...
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                ...
                'subjects' => array(array('course_id' => 109, 'title' => 'الرياضيات',
                                          ...
                                          'mastery' => array('open' => 12, 'mastered' => 7, 'percent' => 58))),
                'sessions' => array(), 'teacher_notes' => array(), 'payments' => array(),
                'quiz_results' => array(array(
                    'lesson' => array('id' => 91, 'title' => 'الكسور المتكافئة'),
                    'course' => array('id' => 39, 'title' => 'الرياضيات — الصف الرابع'),
                    'score' => 2, 'total' => 3, 'passed' => true, 'tries' => 4,
                    'submitted_at' => '2026-09-05T19:12:00+03:00',
                )),
            ),
            'message' => '', 'meta' => array(),
        )))),
    ...
    'tags' => array('Parent'),
    'summary' => 'Set the weekly day plan for one child',
    'description' => implode("\n", array(
        '`plan_days` 1–7, or `null` for "not set" (the default of 5 is then used and announced as such).',
        '',
        'This writes the plan of **this child only**. It does not go through the settings save, which',
        'writes every alert key from the same body and would switch off whatever was not sent.',
    )),
    'security' => $auth,
    'parameters' => array(array('name' => 'id', 'in' => 'path', 'required' => true, 'schema' => array('type' => 'integer'))),
    'requestBody' => array('required' => true, 'content' => array('application/json' => array(
        'schema' => array('type' => 'object', 'required' => array('plan_days'), 'properties' => array(
            'plan_days' => array('type' => array('integer', 'null'), 'minimum' => 1, 'maximum' => 7),
        )),
        'example' => array('plan_days' => 4),
    ))),
    'responses' => array(
        '200' => array('description' => 'Saved', 'content' => array('application/json' => array('example' => array(
            'data' => array('student_id' => 403, 'plan_days' => 4, 'plan_is_default' => false),
            'message' => 'حفظت خطة الأسبوع.', 'meta' => new stdClass(),
        )))),
        '401' => $r_401, '403' => $r_403,
        '404' => $err_ref('No active link with that child.',
            array('message' => 'لا رابط نشط بهذا الابن في حسابك.', 'code' => 'not_found')),
        '422' => $r_422, '429' => $r_429,
    ),
)),
...
'/api/v1/parent/link' => array('post' => array(
    ...
    'description' => implode("\n", array(
        ...
        '',
        '**One reply for every case (TQ-LINK-ENUM)**: an unknown e-mail, a teacher, a disabled account and',
        'a real student all return the same `201 {status: "pending"}` with the same neutral sentence —',
        'and **no `link_id`**. It used to be `0` for an unknown address and a real number for a real',
        'student, so the reply itself said who has an account. Read the request back from',
        '`GET /parent/children` (it shows by e-mail, without the name, until the child consents).',
        '',
        'A 409 is kept only where nothing is revealed: the child is already actively linked to you, or',
        'the address is your own. A 422 when the identifier is not an e-mail.',
    )),
    'security' => $auth,
    'requestBody' => array('required' => true, 'content' => array('application/json' => array(
    ...
    'responses' => array(
        '201' => array('description' => 'Requested', 'content' => array('application/json' => array('example' => array(
            ...
            'message' => 'إن كان هذا البريد لحساب طالب مفعل في تقدر فقد وصله طلب الربط بنص الموافقة، ويظهر ابنك في حسابك بعد أن يوافق من حسابه. ولا يفتح شيء من بياناته قبل ذلك.',
            'meta' => array(),
        )))),
        '401' => $r_401, '403' => $r_403, '422' => $r_422,
        '409' => $err_ref('Already actively linked to you, your own address, or too many requests.',
                          array('message' => 'هذا الحساب مرتبط بحسابك بالفعل.',
                                'code' => 'link_failed')),
        '429' => $r_429,
    ),
)),
...
'/api/v1/teacher/settings' => array(
    'get' => array(
        ...
        'description' => implode("\n", array(
            ...
            '',
            '**Parent: `interrupts` is the list to show (TQ-PARENT-INTERRUPTS).** It is built from the store',
            'that actually decides whether a parent alert is written at all (`weekly` plus the events of',
            '`notify_keys()`). The `notify` matrix is per-channel delivery and no longer offers',
            '`review_due`/`station_unlocked` to a parent — those are student events. `weekly_digest` in the',
            'matrix now also governs the weekly report.',
        )),
        'security' => $auth,
        'responses' => array(
            '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
                'data' => array(
                    'account' => array('id' => 291, 'name' => 'ولي الاختبار', 'role' => 'parent'),
                    'alert_threshold' => 60,
                    'interrupts' => array(
                        array('key' => 'weekly', 'label' => 'التقرير الأسبوعي', 'enabled' => true),
                        array('key' => 'quiz_result', 'label' => 'نتيجة اختبار درس', 'enabled' => true),
                        array('key' => 'exam_result', 'label' => 'نتيجة امتحان', 'enabled' => true),
                        array('key' => 'placement_result', 'label' => 'نتيجة تحديد المستوى', 'enabled' => true),
                        array('key' => 'station_failed', 'label' => 'رسوب في اختبار محطة', 'enabled' => true),
                        array('key' => 'inactivity_3days', 'label' => 'انقطاع ثلاثة أيام', 'enabled' => false),
                        array('key' => 'session_request', 'label' => 'طلب حصة خاصة', 'enabled' => true),
                        array('key' => 'certificate', 'label' => 'شهادة جديدة', 'enabled' => true),
                    ),
                ),
                'message' => '', 'meta' => array('role' => 'parent'),
            )))),
            '401' => $r_401, '403' => $r_403, '429' => $r_429),
    ),
    'patch' => array(
        ...
        'description' => implode("\n", array(
            ...
            '`preferences`, and for a parent `interrupts`. Anything else is a 422 that names them.',
            '',
            '`interrupts` is a **partial** update: `{"section":"interrupts","interrupts":{"quiz_result":false}}`',
            'changes that key only and returns the full list. An unknown key is a 422.',
            '',
        ...
        'requestBody' => array('required' => true, 'content' => array('application/json' => array(
            'schema' => array('type' => 'object', 'required' => array('section'), 'properties' => array(
                'section' => array('type' => 'string',
                    'enum' => array('profile', 'password', 'notifications', 'preferences', 'interrupts')),
                'interrupts' => array('type' => 'object', 'additionalProperties' => array('type' => 'boolean'),
                    'description' => 'Parent, section `interrupts` only.'),
            )),
            'example' => array('section' => 'interrupts', 'interrupts' => array('weekly' => true, 'quiz_result' => false)),
        ))),
        'responses' => array(
            '200' => array('description' => 'Saved. For `interrupts` the full list comes back.',
                'content' => array('application/json' => array('example' => array(
                'data' => array('interrupts' => array(
                    array('key' => 'weekly', 'label' => 'التقرير الأسبوعي', 'enabled' => true),
                    array('key' => 'quiz_result', 'label' => 'نتيجة اختبار درس', 'enabled' => false),
                )),
                'message' => 'حفظت تفضيلاتك.', 'meta' => new stdClass(),
            )))),
            '401' => $r_401, '403' => $r_403, '422' => $r_422, '429' => $r_429,
...
'/api/v1/parent/pay#get' => array('get' => array(
    ...
    'description' => implode("\n", array(
        ...
        '',
        '`course_offers` and `book_offers` are what `POST /parent/pay` accepts with `kind: course|book`',
        '(`id` goes in `course_id`/`book_id`). They come from the same `offers()` the purchase engine',
        'charges by, so the price shown is the price invoiced. `access_days: 0` means lifetime access.',
        'Both are empty when single-course or book sales are switched off.',
    )),
    'security' => $auth,
    'responses' => array(
        '200' => array('description' => 'OK', 'content' => array('application/json' => array('example' => array(
            'data' => array(
                ...
                'course_offers' => array(array(
                    'id' => 109, 'title' => 'الرياضيات — الفصل الأول',
                    'price' => array('amount' => 20000, 'decimal' => '200.00', 'currency' => 'SAR', 'formatted' => '200.00 ر.س'),
                    'list_price' => null, 'access_days' => 365,
                )),
                'book_offers' => array(array(
                    'id' => 14, 'title' => 'كتاب الطالب — الرياضيات',
                    'price' => array('amount' => 4500, 'decimal' => '45.00', 'currency' => 'SAR', 'formatted' => '45.00 ر.س'),
                    'list_price' => null, 'access_days' => 0,
                )),
            ),
            'message' => '',
