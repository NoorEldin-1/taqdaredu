...
$tq_keys = array(
    'test_secret' => array('المفتاح السري — اختبار', 'sk_test_…', 'وهو المفتاح الذي يوقع الطلبات. لا يعرض للمتصفح أبدا.'),
    'test_public' => array('المفتاح العام — اختبار', 'pk_test_…', 'يعرض للمتصفح، ولا ضرر في ظهوره.'),
    'live_secret' => array('المفتاح السري — إنتاج', 'sk_live_…', 'به تخصم أموال حقيقية. اقصر من يصل إلى هذه الشاشة.'),
    'live_public' => array('المفتاح العام — إنتاج', 'pk_live_…', 'يعرض للمتصفح، ولا ضرر في ظهوره.'),
);

$tq_states = array(
    'initiated' => array('warn',   'لم تكتمل'),
    'paid'      => array('ok',     'دفعت'),
    'failed'    => array('danger', 'لم تنجح'),
    'mismatch'  => array('danger', 'مبلغ لا يطابق'),
);
...
<?php tqa_head('بوابة الدفع — تاب',
    'الدفع بالبطاقة: مدى وفيزا وماستركارد. المفاتيح تحفظ هنا، والدفعات تسوى من رد تاب لا من رد المتصفح.',
    'card',
...
<?php if ($live): ?>
    <div class="tqa-note tqa-section">
        ...
        <span>
            <strong>الدفع بالبطاقة شغال في وضع الإنتاج.</strong>
            كل دفعة تخصم مالا حقيقيا، ومن يدفع يفعل اشتراكه بنفسه بلا تفعيل يدوي.
        </span>
    </div>
<?php elseif ($testing): ?>
    <div class="tqa-note tqa-note--warn tqa-section">
        ...
        <span>
            <strong>وضع الاختبار — لا يحصل مالا.</strong>
            الطالب يرى خيار البطاقة ويدفع وينجح الدفع ظاهريا، ولا يصل ريال واحد.
            حول الوضع إلى «إنتاج» قبل الإطلاق، وتأكد أن مفتاح الإنتاج محفوظ.
        </span>
    </div>
<?php elseif ($on): ?>
    <div class="tqa-note tqa-note--warn tqa-section">
        <span aria-hidden="true"><?php echo tq_icon('alert', 18); ?></span>
        <span>
            <strong>مفعلة بلا مفتاح سري.</strong>
            لا مفتاح سري محفوظ لوضع <?php echo $mode === 'live' ? 'الإنتاج' : 'الاختبار'; ?>،
...
<?php else: ?>
    <div class="tqa-note tqa-section">
        ...
        <span>
            <strong>البوابة معطلة.</strong>
            الطالب يرى التحويل البنكي وحده في شاشة الاشتراك — لا خيار بطاقة ولا زر
            ينتهي إلى خطأ. وهذه حال سليمة تماما لمنصة لم تفتح حساب تاب بعد.
        </span>
    </div>
<?php endif; ?>
...
<div class="tqa-grid tqa-grid--3" style="margin-block-end:var(--tq-space-xl)">
    <div class="tqa-stat">
        <div class="tqa-stat__top">
            <span class="tqa-stat__label">دفعات نجحت</span>
    ...
    <div class="tqa-stat">
        <div class="tqa-stat__top">
            <span class="tqa-stat__label">لم تكتمل</span>
        ...
        <span class="tqa-stat__hint">بدأت ولم تنته — يسألها الكرون كل ربع ساعة</span>
    </div>

    <div class="tqa-stat">
        <div class="tqa-stat__top">
            <span class="tqa-stat__label">أخفقت أو لم تطابق</span>
...
<div class="tqa-cols">
    <div class="tqa-stack">
        ...
        <form class="tqa-card" method="post" action="<?php echo site_url('taqdar_admin/tap_save'); ?>">
            ...
            <div class="tqa-card__head" style="padding:0 0 var(--tq-space-l);margin-block-end:var(--tq-space-l)">
                ...
                <span class="tqa-row" style="margin-inline-start:auto;gap:var(--tq-space-xs)">
                    <span class="tqa-badge tqa-badge--<?php echo $ready ? 'ok' : 'muted'; ?>">
                        <?php echo $ready ? 'شغالة' : 'لا تعرض للطالب'; ?>
                    </span>
                    <?php if ($testing): ?>
                        <span class="tqa-badge tqa-badge--danger">وضع اختبار</span>
                    <?php endif; ?>
                </span>
            </div>
            ...
            <div class="tqa-prefrow">
                <div class="tqa-prefrow__main">
                    <label class="tqa-prefrow__title" for="tap_on">تفعيل الدفع بالبطاقة</label>
                    <span class="tqa-prefrow__hint">
                        المعطلة لا تظهر للطالب أصلا، ويبقى التحويل البنكي وحده.
                    </span>
                </div>
            ...
            <div class="tqa-fieldgrid" style="margin-block-start:var(--tq-space-l)">
                <div class="tqa-field">
                    <label class="tqa-field__label" for="tap_mode">
                        الوضع <span class="tqa-field__req" aria-hidden="true">*</span>
                    </label>
                    <select class="tqa-select" id="tap_mode" name="tq_tap_mode" required>
                        ...
                        <option value="live" <?php echo $mode === 'live' ? 'selected' : ''; ?>>
                            إنتاج — يخصم مالا حقيقيا
                        </option>
                    </select>
                    <span class="tqa-field__hint">
                        الوضع يختار المفتاح المستعمل. ومفتاح اختبار في وضع إنتاج يعني
                        دفعا ينجح بلا مال — فافحص المفتاح بالزر أدناه بعد الحفظ.
                    </span>
                </div>

                <div class="tqa-field">
                    <label class="tqa-field__label" for="tap_merchant">معرف التاجر</label>
                    ...
                    <span class="tqa-field__hint">
                        اختياري — من لوحة تاب (Merchant ID). يرسل مع كل دفعة إن ملئ.
                    </span>
                </div>

                <div class="tqa-field">
                    <label class="tqa-field__label">عملة الدفع</label>
                    <input class="tqa-input tqa-input--ltr" type="text" dir="ltr" readonly
                           value="<?php echo html_escape($cfg['currency']); ?>">
                    <span class="tqa-field__hint">
                        تقرأ من «عملة النظام» في
                        <a href="<?php echo site_url('admin/payment_settings'); ?>">إعدادات الدفع</a> —
                        فلا تختلف عما عرض على الطالب.
                    </span>
                </div>
            </div>

            <?php foreach (array('test' => 'مفاتيح الاختبار', 'live' => 'مفاتيح الإنتاج') as $tq_m => $tq_h): ?>
                <h3 class="tqa-field__label" style="margin-block:var(--tq-space-l) var(--tq-space-s)">
                    <?php echo html_escape($tq_h); ?>
                    <?php if ($mode === $tq_m): ?>
                        <span class="tqa-badge tqa-badge--ok">الوضع الجاري</span>
                    <?php endif; ?>
                </h3>
                    ...
                    ?>
                        <div class="tqa-field">
                            <label class="tqa-field__label" for="k_<?php echo $tq_k; ?>">
                                ...
                                <?php if ($tq_has): ?>
                                    <span class="tqa-badge tqa-badge--ok">محفوظ</span>
                                <?php endif; ?>
                            </label>
            ...
            <div class="tqa-actions">
                <button type="submit" class="tqa-btn tqa-btn--primary"
                        data-tqa-confirm-title="حفظ إعدادات بوابة الدفع"
        ...
        <div class="tqa-card">
            ...
            <p class="tqa-field__hint" style="margin-block-end:var(--tq-space-m)">
                يسأل تاب عن دفعة غير موجودة بالمفتاح المحفوظ: المفتاح المرفوض يرد
                «غير مصرح»، والمقبول يرد «غير موجودة». فيعرف أن المفتاح يعمل قبل
                أن يجربه مشتر.
            </p>
            <div class="tqa-row" style="gap:var(--tq-space-s);flex-wrap:wrap">
                <?php foreach (array('test' => 'افحص مفتاح الاختبار', 'live' => 'افحص مفتاح الإنتاج') as $tq_m => $tq_l): ?>
    ...
    <aside>
        <div class="tqa-note tqa-note--warn">
            ...
            <span>
                المفاتيح تحفظ في قاعدة البيانات لا في الشيفرة — والمستودع عام، والنشر
                يمسح ما فيه من تعديل. ومن يصل إلى هذه الشاشة يصل إليها، فاقصر
                صلاحية «تقدر» على من يحتاجها من
                <a href="<?php echo site_url('admin/admins'); ?>">المسؤولين</a>.
            </span>
        </div>
        ...
        <div class="tqa-note" style="margin-block-start:var(--tq-space-l)">
            ...
            <span>
                <strong>عنوان الويبهوك</strong> — يرسل مع كل دفعة تلقائيا، ولا يحتاج
                تسجيلا في لوحة تاب:
                <br><code class="tq-ltr" dir="ltr" style="font-size:12px"><?php echo html_escape($tq_hook); ?></code>
                <?php if (!$tq_public): ?>
                    <br><strong>ولا يرسل من هذا الخادم:</strong> العنوان محلي أو بلا
        ...
        <div class="tqa-note" style="margin-block-start:var(--tq-space-l)">
            <span aria-hidden="true"><?php echo tq_icon('check-badge', 18); ?></span>
            <span>
                <strong>ما يقع عند الدفع:</strong> يصدر الاشتراك والفاتورة أولا، ثم
                تفتح صفحة تاب. وحين يرجع الطالب تسأل تاب عن الدفعة بمفتاحك، ويقابل
                مبلغها مبلغ الفاتورة، ثم يفعل الاشتراك ويخطر صاحبه. فلا يفعل شيء
                برابط عودة مصنوع، ولا بمبلغ يخالف الفاتورة.
            </span>
        </div>
        ...
        <div class="tqa-note" style="margin-block-start:var(--tq-space-l)">
            ...
            <span>
                من دفع وأغلق المتصفح قبل أن يعود يفعل اشتراكه بالويبهوك. ولو لم يصل
                الويبهوك يقرأ الكرون
                (<code class="tq-ltr" dir="ltr" style="font-size:11px">taqdar_cron reconcile</code>)
                ما بقي «لم يكتمل» ويسأل تاب عنه — فلا تبقى دفعة حصلت بلا اشتراك.
            </span>
        </div>
    </aside>
</div>
...
<?php if (!$attempts): ?>
    ...
    <?php tqa_empty('لا محاولة دفع بعد',
        'يظهر هنا صف لكل دفعة يبدؤها طالب، بحالها عند تاب ومبلغها ورقم فاتورتها.',
        '', '', 'card'); ?>
            ...
            <tr>
                ...
                <td data-label="الوضع">
                    <span class="tqa-badge tqa-badge--<?php echo $a['mode'] === 'live' ? 'ok' : 'muted'; ?>">
                        <?php echo $a['mode'] === 'live' ? 'إنتاج' : 'اختبار'; ?>
                    </span>
                </td>
                ...
                <td data-label="مراجعة">
                    <?php if ($a['status'] === 'paid'): ?>
                        <span style="color:var(--tq-text3)">سويت</span>
                    <?php elseif (empty($a['charge_id'])): ?>
                        <span style="color:var(--tq-text3)">لم تبلغ البوابة</span>
                    <?php else: ?>
...
<div class="tqa-note" style="margin-block-start:var(--tq-space-l)">
    <span aria-hidden="true"><?php echo tq_icon('alert', 18); ?></span>
    <span>
        «مبلغ لا يطابق» صف يستحق أن يقرأ فورا: وصلت دفعة قيمتها تخالف قيمة الفاتورة،
        فلم يفعل الاشتراك عمدا. راجعها في لوحة تاب قبل أن تفعل شيئا — والاسترداد يجري
        من هناك لا من هنا.
    </span>
</div>
