 ...
 * تعديل مسؤول.
 *
 * الشكل نفسه الذي في [admin_add.php]، ولنفس السبب: كان معالج أربع خطوات
 * يخرج بطاقة بيضاء متى تعثر سكربت القالب (انظر TQ-WIZARD-BLANK هناك).
 *
 * وحقل «المهارات» كان `data-role="tagsinput"` من
 * `bootstrap-tagsinput` — مكتبة تجلب من CDN، فتحتاج شبكة، وترسم الحبات
 * **حمراء بعلامة ×**: لون خطر لقيمة عادية. صار حقل وسوم مبنيا هنا،
 * وقيمته تبقى في `<input type="hidden">` كما كانت — مفصولة بفواصل — فلا
 * يتغير شيء في جانب الخادم.
 */
$tq_id     = (int) $user_data['id'];
$tq_name   = trim($user_data['first_name'] . ' ' . $user_data['last_name']);
$tq_photo  = $this->user_model->get_user_image_url($tq_id);
?>

<?php tqa_head('تعديل المسؤول', $tq_name !== '' ? $tq_name : $user_data['email'], 'shield',
    '<a class="tqa-btn tqa-btn--ghost" href="' . site_url('admin/permissions?permission_assing_to=' . $tq_id) . '">'
  . tq_icon('key', 16) . ' الصلاحيات</a>'
...
<form action="<?php echo site_url('admin/admins/edit/' . $tq_id); ?>" enctype="multipart/form-data" method="post"
        ...
        <div class="tqa-fieldgrid">
            <div class="tqa-field">
                ...
                <input class="tqa-input" type="text" id="first_name" name="first_name" required maxlength="100"
                       value="<?php echo html_escape($user_data['first_name']); ?>">
            </div>
            ...
            <div class="tqa-field">
                ...
                <input class="tqa-input" type="text" id="last_name" name="last_name" required maxlength="100"
                       value="<?php echo html_escape($user_data['last_name']); ?>">
            </div>
            ...
            <div class="tqa-field tqa-field--full">
                ...
                <input class="tqa-input" type="text" id="title" name="title" maxlength="255"
                       value="<?php echo html_escape($user_data['title']); ?>">
            </div>

            <?php /* حقل الوسوم: القيمة الحقيقية في الحقل المخفي، والحبات
                     مرآة له. وبلا جافاسكربت يبقى الحقل المخفي محملا بقيمته
                     المحفوظة فيرسل كما هو — لا يفقد التعديل ما لم يحرر. */ ?>
            <div class="tqa-field tqa-field--full">
                <label class="tqa-field__label" for="skills_in">المهارات</label>
                <div class="tqa-tags" data-tqa-tags>
                    <input type="hidden" name="skills" value="<?php echo html_escape($user_data['skills']); ?>"
                           data-tqa-tags-value>
                    <input class="tqa-tags__in" type="text" id="skills_in"
                           placeholder="اكتب مهارة ثم اضغط Enter" autocomplete="off" data-tqa-tags-input>
                </div>
                <span class="tqa-field__hint">تفصل بفواصل. اضغط Enter بعد كل واحدة.</span>
            </div>

            <div class="tqa-field tqa-field--full">
                <label class="tqa-field__label" for="biography">نبذة</label>
                <textarea class="tqa-textarea" id="biography" name="biography" rows="4"><?php
                    echo html_escape($user_data['biography']); ?></textarea>
            </div>

            <div class="tqa-field tqa-field--full">
                <span class="tqa-field__label">الصورة الشخصية</span>
                <div class="tqa-file">
                    <img class="tqa-avatar" src="<?php echo html_escape($tq_photo); ?>" alt="الصورة الحالية"
                         width="38" height="38">
                    ...
                    <span class="tqa-file__name" data-tqa-file-name>اترك الحقل فارغا لإبقاء الصورة الحالية</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="tqa-fieldgrid">
            <div class="tqa-field">
                ...
                <input class="tqa-input tqa-input--ltr" type="email" id="email" name="email" required dir="ltr"
                       value="<?php echo html_escape($user_data['email']); ?>">
            </div>

            <div class="tqa-field">
                <label class="tqa-field__label" for="password">كلمة مرور جديدة</label>
                <input class="tqa-input tqa-input--ltr" type="password" id="password" name="password" dir="ltr"
                       minlength="8" autocomplete="new-password">
                <span class="tqa-field__hint">اتركها فارغة لإبقاء كلمة المرور الحالية.</span>
            </div>
        </div>
    </div>
        ...
        <div class="tqa-fieldgrid tqa-fieldgrid--3">
            <div class="tqa-field">
                ...
                <input class="tqa-input tqa-input--ltr" type="url" id="facebook_link" name="facebook_link" dir="ltr"
                       value="<?php echo html_escape($social_links['facebook']); ?>">
            </div>
            <div class="tqa-field">
                ...
                <input class="tqa-input tqa-input--ltr" type="url" id="twitter_link" name="twitter_link" dir="ltr"
                       value="<?php echo html_escape($social_links['twitter']); ?>">
            </div>
            <div class="tqa-field">
                ...
                <input class="tqa-input tqa-input--ltr" type="url" id="linkedin_link" name="linkedin_link" dir="ltr"
                       value="<?php echo html_escape($social_links['linkedin']); ?>">
            </div>
        </div>
    </div>
    ...
    <div class="tqa-actions">
        ...
        <a class="tqa-btn tqa-btn--ghost" href="<?php echo site_url('admin/admins'); ?>">إلغاء</a>
    </div>
</form>
