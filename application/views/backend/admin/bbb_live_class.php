<?php $tq_course_id = $course_details['id'] ?? 0; ?>
<?php $bbb_meeting = $this->db->where('course_id', $tq_course_id)->get('bbb_meetings')->row_array(); ?>
<div class="row">
    <div class="col-md-6">
...
<script>
    ...
    function save_bbb_meeting(){
        ...
        if (bbb_moderator_pw == '' || bbb_viewer_pw == '' || bbb_meeting_id == '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo site_url('admin/save_bbb_meeting/' . $tq_course_id); ?>',
                data: {
    ...
    function start_bbb_meeting() {
        $.ajax({
            type: 'GET',
            url: '<?php echo site_url('admin/start_bbb_meeting/' . $tq_course_id); ?>',
            success: function(response) {
