...
list($tone, $label) = isset($states[$st]) ? $states[$st] : array('muted', 'غير معروفة');

$ch       = (string) ($p['requested_channel'] ?: $p['payment_type']);
$ch_meta  = isset($channels[$ch]) ? $channels[$ch] : null;
$ch_label = $ch_meta ? $ch_meta['label'] : 'قناة تحدد مع الإدارة';
...
<?php tqa_head('طلب سحب #' . (int) $p['id'],
               'كل ما يلزم لاتخاذ القرار وللإجابة عنه بعده.', 'send',
               '<a class="tqa-btn tqa-btn--ghost" href="' . site_url('taqdar_admin/payouts') . '">رجوع إلى القائمة</a>'); ?>
...
<?php if ($st === 0 && !empty($d['dest_changed'])): ?>
    <div class="tqa-note tqa-note--warn" style="margin-block-end:var(--tq-space-l)">
        ...
        <span>
            <b>الوجهة تغيرت عن آخر تحويل.</b>
            كانت <span class="tqa-num"><?php echo html_escape((string) $h['last_dest']); ?></span>
            وصارت <span class="tqa-num"><?php echo html_escape((string) $p['destination']); ?></span>.
            وهذا يقع بلا سبب سيئ في الغالب — تغيير بنك أو محفظة. لكن حسابا اخترق يبدأ من هنا،
            فتأكد بالاتصال قبل التحويل ولا تكتف بالرسالة.
        </span>
    </div>
<?php elseif ($st === 0 && !empty($d['first_request'])): ?>
    <div class="tqa-note" style="margin-block-end:var(--tq-space-l)">
        <span aria-hidden="true"><?php echo tq_icon('alert', 18); ?></span>
        <span>
            <b>أول طلب لهذا المعلم.</b>
            لا وجهة سابقة يقارن بها. تحقق من الاسم على الحساب قبل التحويل — الحوالة الأولى
            هي التي تصحح، وما بعدها يقارن بها.
        </span>
    </div>
<?php endif; ?>
...
<div class="tqa-grid tqa-grid--3" style="margin-block-end:var(--tq-space-xl)">
    <div class="tqa-stat">
        <div class="tqa-stat__top">
            <span class="tqa-stat__label">المطلوب</span>
    ...
    <div class="tqa-stat">
        <div class="tqa-stat__top">
            <span class="tqa-stat__label">محجوز الآن</span>
    ...
    <div class="tqa-stat">
        <div class="tqa-stat__top">
            <span class="tqa-stat__label">رصيده بعد الحجز</span>
...
<div class="tqa-cols">
    ...
    <div>
        ...
        <div class="tqa-card" style="margin-block-end:var(--tq-space-l)">
            <div class="tqa-card__head"><h2 class="tqa-card__title">إلى أين تحول</h2></div>
            <div class="tqa-card__body">
                <dl class="tqa-dl">
                    ...
                    <dd>
                        ...
                        <?php if ($country === 'eg'): ?>
                            <span class="tqa-badge tqa-badge--warn">قناة مصرية</span>
                        <?php endif; ?>
                    </dd>
                    ...
                    <dd>
                        <?php if (!empty($p['destination'])): ?>
                            ...
                            <button type="button" class="tqa-btn tqa-btn--ghost tqa-btn--sm"
                                    data-tqa-copy="<?php echo html_escape((string) $p['destination']); ?>">انسخ</button>
                        <?php else: ?>
                            <span class="tqa-dim">لم يحدد وجهة — اسأله قبل التحويل</span>
                        <?php endif; ?>
                    </dd>
                ...
                <?php if ($country === 'eg'): ?>
                    <p class="tqa-reach__note" style="margin-block-start:var(--tq-space-m)">
                        الرصيد بالريال والوجهة مصرية — فالتحويل يمر بصرف. سجل رقم العملية وسعر
                        الصرف في الملاحظة أدناه، فالمعلم يرى مبلغه بالريال ويستلم بالجنيه.
                    </p>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="tqa-card" style="margin-block-end:var(--tq-space-l)">
            <div class="tqa-card__head">
                <h2 class="tqa-card__title">من أين جاء هذا الرصيد</h2>
            </div>
            <div class="tqa-card__body">
                <?php if (!$d['sources']): ?>
                    <p class="tqa-dim">لا مبيعات مقيدة في دفتر هذا المعلم بعد.</p>
                <?php else: ?>
                    <p class="tqa-reach__lead" style="margin-block-end:var(--tq-space-m)">
                        آخر المبيعات وحصته الصافية من كل منها بعد عمولة المنصة — وهي نفسها
                        الأرقام التي يراها في كشف حسابه، فلا يفترق ما يقرؤه عما تقرؤه.
                    </p>
                    <table class="tqa-split__table">
                        <thead>
                            <tr><th>التاريخ</th><th>ما بيع</th><th>النوع</th><th>حصته الصافية</th></tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php echo $s['kind'] === 'plan'
                                        ? '<span class="tqa-badge tqa-badge--ok">باقة</span>'
                                        : '<span class="tqa-badge tqa-badge--muted">كورس</span>'; ?>
                                </td>
        ...
        <?php if ($d['entries']): ?>
            <details class="tqa-card">
                ...
                <div class="tqa-card__body">
                    <p class="tqa-reach__lead" style="margin-block-end:var(--tq-space-m)">
                        القيود كما كتبت. القيد لا يعدل ولا يحذف — يقابل بعكسه، فالسالب هنا
                        حقيقة محاسبية لا خطأ.
                    </p>
                    <table class="tqa-split__table">
                        <thead><tr><th>الوقت</th><th>النوع</th><th>الدلو</th><th>المبلغ</th><th>عن</th></tr></thead>
                        <tbody>
    ...
    <aside>
        ...
        <?php if ($st === 0): ?>
            <form class="tqa-card" method="post" action="<?php echo site_url('taqdar_admin/payout_decide'); ?>"
                ...
                <div class="tqa-card__head"><h2 class="tqa-card__title">القرار</h2></div>
                <div class="tqa-card__body">
                    <div class="tqa-field">
                        <label class="tqa-field__label" for="tqa-ref">رقم العملية أو سبب الرفض</label>
                        <input class="tqa-input tqa-input--ltr" dir="ltr" id="tqa-ref" type="text" name="reference">
                        <span class="tqa-field__hint">
                            عند الاعتماد: رقم الحوالة كما في كشف البنك — وهو شرط، فتحويل بلا
                            مرجع لا يطابق. وعند الرفض: السبب كما سيصل المعلم في إشعاره.
                        </span>
                    </div>

                    <div class="tqa-field">
                        <label class="tqa-field__label" for="tqa-note">ملاحظة داخلية</label>
                        <input class="tqa-input" id="tqa-note" type="text" name="admin_note">
                        <span class="tqa-field__hint">
                            لا تعرض للمعلم. لسعر الصرف، أو لمن اتصلت به، أو لما تريد أن تذكره
                            حين يسأل أحد بعد شهر.
                        </span>
                    </div>
                    ...
                    <div style="display:flex;gap:var(--tq-space-s);flex-wrap:wrap">
                        <button class="tqa-btn tqa-btn--mastery" type="submit" name="act" value="pay"
                                ...
                                data-tqa-confirm-ok="نعم، حولت">حولت</button>
                        <button class="tqa-btn tqa-btn--ghost" type="submit" name="act" value="reject"
                                ...
                                data-tqa-confirm-tone="danger">رفض</button>
                    </div>
                </div>
            </form>
        <?php else: ?>
            <div class="tqa-card" style="margin-block-end:var(--tq-space-l)">
                <div class="tqa-card__head"><h2 class="tqa-card__title">أثر القرار</h2></div>
                <div class="tqa-card__body">
                    <dl class="tqa-dl">
                        <dt>الحال</dt>
                        <dd><span class="tqa-badge tqa-badge--<?php echo $tone; ?>"><?php echo html_escape($label); ?></span></dd>
                        <dt>من قرر</dt>
                        <dd><?php echo html_escape(trim((string) $p['decided_name']) ?: 'غير مسجل'); ?></dd>
                        <dt>متى</dt>
                    ...
                    <?php if (empty($p['decided_name']) && empty($p['reference'])): ?>
                        <p class="tqa-reach__note">
                            قرر هذا الطلب قبل أن يسجل أثر القرار — فلا اسم ولا مرجع. وما يقرر
                            بعد اليوم يحفظ كلاهما.
                        </p>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <?php /* ---- المعلم وسابقته ---- */ ?>
        <div class="tqa-card" style="margin-block-end:var(--tq-space-l)">
            <div class="tqa-card__head"><h2 class="tqa-card__title">المعلم</h2></div>
            <div class="tqa-card__body">
                <dl class="tqa-dl">
                    <dt>الاسم</dt>
                    ...
                    <dd>
                        <?php echo ((int) $p['teacher_status'] === 1)
                            ...
                            : '<span class="tqa-badge tqa-badge--danger">موقوف</span>'; ?>
                    </dd>
                    <dt>مساراته</dt>
                ...
                <a class="tqa-btn tqa-btn--ghost tqa-btn--sm"
                   href="<?php echo site_url('taqdar_admin/form/users/' . (int) $p['user_id']); ?>">افتح حسابه</a>
            </div>
        </div>
        ...
        <?php if ($d['others']): ?>
            <div class="tqa-card">
                <div class="tqa-card__head"><h2 class="tqa-card__title">طلباته الأخرى</h2></div>
                <div class="tqa-card__body" style="padding:0">
                    <table class="tqa-split__table" style="margin:0">
                        <thead><tr><th>#</th><th>المبلغ</th><th>الحال</th><th>التاريخ</th></tr></thead>
                        <tbody>
