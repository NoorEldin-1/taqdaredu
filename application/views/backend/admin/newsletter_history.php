 ...
 */
$tq_tabs = array(
    'pending' => array('بانتظار الإرسال', 'warn'),
    'sent'    => array('أرسلت',           'ok'),
    'faild'   => array('تعثرت',           'danger'),
    'unable'  => array('تعذر إرسالها',    'muted'),
);
...
<?php tqa_head('سجل إرسال النشرة',
    'كل رسالة وحالتها. والمتعثرة تعاد يدويا من هنا.',
    'receipt',
    '<a class="tqa-btn tqa-btn--ghost" href="' . site_url('admin/newsletters') . '">'
  . tq_icon('chev-prev', 16) . ' قوالب النشرة</a>'); ?>

<nav class="tqa-tabs" aria-label="تصفية السجل بالحالة">
...
<form class="tqa-toolbar" method="get" action="<?php echo site_url('admin/newsletter_history/' . $type); ?>">
    <label class="tqa-sr" for="q">ابحث في السجل</label>
    <input class="tqa-input" type="search" id="q" name="q" placeholder="ابحث ببريد أو عنوان…"
...
<?php if (empty($rows)): ?>
    ...
    <?php tqa_empty(
        $search !== '' ? 'لا نتيجة لهذا البحث' : 'لا رسائل بهذه الحالة',
        $search !== ''
            ? 'جرب جزءا من البريد أو العنوان.'
            : 'يمتلئ هذا الجدول حين ترسل نشرة — ولا يضاف إليه بيد.',
        '', '', 'receipt'
    ); ?>
...
<?php else: ?>
    ...
    <div class="tqa-table__wrap">
        <table class="tqa-table">
            <caption class="tqa-sr">سجل رسائل النشرة وحالة كل رسالة</caption>
            <thead>
                <tr>
                    <th style="inline-size:60px">#</th>
                    <th>العنوان</th>
                    <th>المستقبل</th>
                    <th style="inline-size:130px">الحالة</th>
                ...
                <tr>
                    ...
                    <td data-label="إجراءات">
                        ...
                        <form method="post"
                              ...
                              data-tqa-confirm-title="<?php echo $tq_r['status'] === 'sent' ? 'إعادة الإرسال' : 'إرسال الآن'; ?>"
                            ...
                            <button type="submit" class="tqa-btn tqa-btn--ghost tqa-btn--sm">
                                ...
                                <?php echo $tq_r['status'] === 'sent' ? 'أرسل ثانية' : 'أرسل الآن'; ?>
                            </button>
                        </form>
                    </td>
                </tr>
    ...
    ?>
    <nav class="tqa-pager" aria-label="صفحات السجل">
        ...
        <?php if ($page_no > 1): ?>
            <a href="<?php echo $tq_url($type, $page_no - 1); ?>" rel="prev" aria-label="الصفحة السابقة">
        ...
        <?php if ($page_no < $page_count): ?>
            <a href="<?php echo $tq_url($type, $page_no + 1); ?>" rel="next" aria-label="الصفحة التالية">
