...
if (!$tq_cat) {
    tqa_head('قسم غير موجود', '', 'grid');
    echo '<div class="tqa-card tqa-card--flush">';
    tqa_empty('لا قسم بهذا المعرف', 'قد يكون حذف من شاشة أخرى.', 'العودة إلى الأقسام', site_url('admin/categories'), 'grid');
    echo '</div>';
    return;
}
...
<?php tqa_head('تعديل القسم', html_escape($tq_cat['name']), 'grid',
    ...
    <form action="<?php echo site_url('admin/categories/edit/' . (int) $category_id); ?>" method="post"
        ...
        <div class="tqa-fieldgrid">
            ...
            <div class="tqa-field tqa-field--full" data-tqa-cover="parent" <?php echo $tq_is_sub ? 'hidden' : ''; ?>>
                ...
                <div class="tqa-file">
                    <?php if ($tq_par_img !== ''): ?>
                        <img class="tqa-file__preview" src="<?php echo html_escape($tq_par_img); ?>" alt="الغلاف الحالي">
                    <?php endif; ?>
                    ...
                    <label class="tqa-file__btn" for="cat_cover">
                        ...
                        <?php echo $tq_par_img !== '' ? 'استبدل الصورة' : 'اختر صورة'; ?>
                    </label>
                    <span class="tqa-file__name" data-tqa-file-name>
                        <?php echo $tq_par_img !== '' ? 'اترك الحقل فارغا لإبقاء الصورة الحالية' : 'لم تختر ملفا بعد'; ?>
                    </span>
                </div>
            ...
            <div class="tqa-field tqa-field--full" data-tqa-cover="sub" <?php echo $tq_is_sub ? '' : 'hidden'; ?>>
                ...
                <div class="tqa-file">
                    <?php if ($tq_sub_img !== ''): ?>
                        <img class="tqa-file__preview" src="<?php echo html_escape($tq_sub_img); ?>" alt="الأيقونة الحالية">
                    <?php endif; ?>
                    ...
                    <label class="tqa-file__btn" for="cat_cover_sub">
                        <?php echo tq_icon('image', 16); ?>
                        <?php echo $tq_sub_img !== '' ? 'استبدل الصورة' : 'اختر صورة'; ?>
                    </label>
                    <span class="tqa-file__name" data-tqa-file-name>
                        <?php echo $tq_sub_img !== '' ? 'اترك الحقل فارغا لإبقاء الصورة الحالية' : 'لم تختر ملفا بعد'; ?>
                    </span>
                </div>
    ...
    <div class="tqa-card tqa-section" style="max-inline-size:760px;margin-block-start:var(--tq-space-l)">
        <div class="tqa-prefrow">
            <div class="tqa-prefrow__main">
                <span class="tqa-prefrow__title">حذف أيقونة القسم الفرعي</span>
                <span class="tqa-prefrow__hint">يحذف الملف وحده — القسم يبقى كما هو.</span>
            </div>
            <div class="tqa-prefrow__end">
                <form method="post"
                      action="<?php echo site_url('admin/categories/sub_category_image/' . (int) $tq_cat['id']); ?>"
                      data-tqa-confirm-title="حذف الأيقونة"
