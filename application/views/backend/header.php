...
$tqa_name  = $tqa_me ? trim($tqa_me['first_name'] . ' ' . $tqa_me['last_name']) : t('مسؤول');
...
$tqa_hour = (int) date('G');
if     ($tqa_hour < 12) $tqa_greet = t('صباح الخير');
elseif ($tqa_hour < 18) $tqa_greet = t('طاب يومك');
else                    $tqa_greet = t('مساء الخير');
...
<header class="tqa-topbar">
    ...
    <button class="tqa-iconbtn tqa-rail-toggle" type="button" data-tqa-toggle
            aria-label="<?php echo te('فتح القائمة'); ?>" aria-expanded="false" aria-controls="tqa-rail">
