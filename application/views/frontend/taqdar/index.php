...
$tq_site_pages = array('home', 'home_elegant', 'courses_page', 'course_page', 'site_teachers', 'site_students', 'site_parents', 'blogs', 'about_us', 'contact_us', 'site_path', 'login', 'sign_up', 'forgot_password',
    ...
    /* وصفحة كتب الصف معها — وهي الفخ الذي حذر منه السطر اعلاه حرفا:
       ردت 200 وطبعت محتواها كاملا، وفتحت بترويسة الثيم الموروث. */
    'site_books_grade',
    'site_summaries',
    /* TQ-FOUNDATION — قسم التأسيس وصفحة مساره: بابان عامان بجوار
       `/catalog` و`/plans`، وإسقاطهما من هذه القائمة هو الفخ نفسه الذي
       وقعت فيه صفحة كتب الصف — ترد 200 وتطبع محتواها كاملا وتفتح
       بترويسة الثيم الموروث، فيقول `curl` «سليمة» وتقول العين غير ذلك. */
    'site_foundation', 'site_foundation_track',
    /* TQ-FND-PACK — وشاشة تأكيد شراء باقة الحصص معهما. وهي الفخ نفسه
       الذي حذر منه السطر أعلاه، ووقعت فيه هذه الشاشة حرفا: ردت 200
       وطبعت محتواها كاملا — وفتحت بغلاف **البوابة** بلا ورقة
       `site/*.css` واحدة، فخرج قوس الهيرو قبة سوداء تملأ الشاشة ولا
       شيء في أي سجل يقول لماذا. */
    'site_foundation_pack_checkout',
    /* TQ-EXPRESS-PAY — وشاشة دفع فاتورة قائمة (`pay/<رقم>`) معها: أخت
       شاشات التأكيد بمفرداتها (`co-*`)، وخارج هذه القائمة تفتح بغلاف
       البوابة بلا ورقة `site/*.css` — الفخ الذي حذرت منه الأسطر أعلاه. */
    'site_invoice_pay',
    /* معاينة الدرس المجاني: عامة كصفحات الموقع — يفتحها من لا حساب له،
       وهي الوعد المكتوب على شارة «معاينة مجانية». انظر `Preview.php`. */
    'site_preview',
    /* TQ-SOCIAL-COMPLETE — شاشة إكمال البيانات بعد الدخول بجوجل أو أبل.
       وهي أخت `sign_up` و`verification_code`: شاشة رحلة تسجيل لا شاشة
       بوابة، فغلافها غلاف الموقع. واسمها بلا بادئة `tq_` عمدا لذلك —
       القاعدة أعلاه تجعل كل `tq_` شاشة بوابة، وشاشة بوابة هنا تعني
       شريطا جانبيا لدور لم يختر صاحبه صفه بعد. */
    'social_complete',
...
<head>
    ...
    <?php /* TQ-META-CAPI — وأحداث التحويل التي أودعها مسار الكتابة قبل
             تحويلته: عودة الدفع تنتهي بـ`redirect()` فلا قالب لها، وهذه
             هي الصفحة التي تليها. وموضعه هنا لا في شاشة بعينها — فالشاشة
             التي يصل إليها المشتري غدا تطلق حدثها بلا سطر يضاف فيها. */ ?>
    <?php echo tq_meta_flush(); ?>
    <?php /* Clarity — بجوار البكسل لا في الذيل: ذاك يعد الزيارة
             وهذا يسجل ما يفعله صاحبها، والسؤالان يسألان عن الصفحة نفسها.
             والدالة تحرس نفسها من التكرار، وتصمت متى أطفئ من اللوحة،
             ولا تحمل لمن رفض ملفات الارتباط. */ ?>
    <?php echo tq_clarity(); ?>
<?php /* TQ-I18N — قاموس المتصفح.
         ...
         يقرؤه آخرها. */ ?>
    <?php echo tq_i18n_js(); ?>
<?php /* TQ-I18N — ومحرك القاموس بعده مباشرة. القاموس أعلاه بيانات
         (`window.TQ_I18N`)، وهذا ما يحولها إلى `TQ.t()` و`TQ.gateFetch()`.

         TQ-PERF-I18N — و`defer` على الموقع العام وحده، لا في كل مكان.

         السبب أن المستهلك يختلف: قوالب البوابة (`tq_library` و
         `tq_mastery` و`tq_mistakes`) تنادي `TQ.t()` من سكربت سطري وقت
         التحليل، فمؤجل يصل بعد فوات الأوان ويرمي `TQ is not defined`.
         أما قوالب الموقع العام — الرئيسية و`site/*` كلها — ففيها **صفر
         نداء** وقت التحليل. فالملف هناك سبعة كيلوبايت تحجب الرسم ولا
         ينتظرها احد.

         وهو نفسه لا يمس DOM ولا يطلب شبكة عند التحميل: يقرأ
         `window.TQ_I18N` ويعرف دوال، ثم ينتهي. فالتأجيل لا يغير سلوكا،
         انما يزيح حاجبا. */ ?>
    <script src="<?php echo tq_asset('js/tq-i18n.js'); ?>"<?php echo !empty($tq_is_site) ? ' defer' : ''; ?>></script>
</head>
