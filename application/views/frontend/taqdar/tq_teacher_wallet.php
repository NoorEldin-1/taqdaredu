...
$tq_nav   = 'wallet';
$tq_role  = 'teacher';
$tq_title = t('المحفظة والأرباح');
$tq_sub   = t('المتاح والمعلق منفصلان، وكل ريال يتتبع إلى مصدره');
$tq_icon  = 'wallet';
...
$tq_states = array(
    'pending'   => array('due',      t('معلق')),
    'available' => array('progress', t('متاح للسحب')),
    'paid'      => array('mastered', t('حول إليك')),
    'refunded'  => array('late',     t('مسترد')),
);
    ...
    <div>
        ...
        <div class="tq-grid tq-grid--3 tq-section">
            <div class="tq-pastel tq-pastel--mint">
                <div class="tq-row tq-row--between">
                    <span class="tq-pastel__label tq-micro"><?php echo t('متاح للسحب'); ?></span>
                ...
                <p class="tq-pastel__body tq-caption" style="margin:0"><?php echo t('تجاوز نافذة الاسترداد'); ?></p>
            </div>

            <div class="tq-pastel tq-pastel--peach">
                <div class="tq-row tq-row--between">
                    <span class="tq-pastel__label tq-micro"><?php echo t('معلق مؤقتا'); ?></span>
            ...
            <div class="tq-pastel tq-pastel--sky">
                <div class="tq-row tq-row--between">
                    <span class="tq-pastel__label tq-micro"><?php echo t('حول إليك'); ?></span>
                ...
                <p class="tq-pastel__body tq-caption" style="margin:0"><?php echo t('مجموع ما استلمته سابقا'); ?></p>
            </div>
        </div>
        ...
        <section class="tq-section" aria-labelledby="tq-stmt-h">
            <div class="tq-sectionhead">
                <h2 id="tq-stmt-h"><?php echo t('كشف الحساب'); ?></h2>
            ...
            <?php else: ?>
                <div class="tq-card tq-empty">
                    ...
                    <h3 class="tq-empty__title"><?php echo t('لا مبيعات بعد'); ?></h3>
                    <p class="tq-empty__text">
                        <?php echo t('أول عملية بيع في كورساتك ستظهر هنا بمبلغها وعمولة المنصة عليها وحصتك منها، وبتاريخ تحررها من نافذة الاسترداد.'); ?>
                    </p>
                    <a class="tq-btn tq-btn--secondary" href="<?php echo base_url('teacher/courses'); ?>"><?php echo t('كورساتي'); ?></a>
                </div>
            <?php endif; ?>
        </section>
        ...
        <section aria-labelledby="tq-payouts-h">
            <div class="tq-sectionhead"><h2 id="tq-payouts-h"><?php echo t('طلبات السحب السابقة'); ?></h2></div>
            ...
            <?php else: ?>
                <div class="tq-card tq-empty">
                    ...
                    <h3 class="tq-empty__title"><?php echo t('لم تطلب سحبا بعد'); ?></h3>
                    <p class="tq-empty__text"><?php echo t('حين تطلب سحب رصيدك المتاح، يظهر الطلب هنا بتاريخه وقناته ووجهته وحالته.'); ?></p>
                    <a class="tq-btn tq-btn--secondary" href="#tq-withdraw"><?php echo t('طلب سحب'); ?></a>
                </div>
            <?php endif; ?>
        </section>
    </div>
    ...
    <aside class="tq-aside">
        ...
        <?php if ((int) $tq_w['locked'] > 0): ?>
            <div class="tq-pastel tq-pastel--lilac">
                <span class="tq-pastel__label tq-micro"><?php echo t('محجوز الآن'); ?></span>
                <p class="tq-pastel__title" style="margin:var(--tq-space-s) 0 0;font:var(--tq-type-numeralXl)"><?php echo $tq_money($tq_w['locked']); ?></p>
                <p class="tq-pastel__body tq-caption" style="margin:0">
                    <?php echo t('خرج من المتاح مقابل طلبات سحب قائمة، ولم يحول بعد. إلغاء الطلب يعيده إلى المتاح.'); ?>
                </p>
            </div>
        <?php endif; ?>
        ...
        $tq_avail = (int) $tq_w['available'];
        $tq_min   = (int) $tq_w['min_payout'];
        $tq_can_withdraw = ($tq_avail >= $tq_min) && $tq_min > 0;

        /* TQ-WITHDRAW-WHY — السبب يقال في رأس النموذج لا في ذيله.
           كان النموذج كله رماديا بلا كلمة فوقه، والسبب سطر صغير تحت زر
           معطل: فيضغط المعلم الحقل فلا يكتب، ويظن الشاشة معطلة قبل أن يصل
           إلى السطر. وثلاثة أسباب لا واحد، ولكل واحد ما يفعله صاحبه.
           والرصيد السالب يكتب بكلمته لا بعلامة الطرح: `-367` في سطر عربي
           تقفز شرطته إلى يمين الرقم فيقرأ «367-»، وكلمة «عليك» أوضح منها
           لمن ليس محاسبا. */
        $tq_block_title = '';
        $tq_block_text  = '';
        if (!$tq_can_withdraw) {
            if ($tq_min <= 0) {
                $tq_block_title = t('طلبات السحب متوقفة مؤقتا');
                $tq_block_text  = t('لم يضبط حد السحب في المنصة بعد، فلا يستقبل النموذج طلبا الآن. رصيدك محفوظ في دفترك كما هو — وتواصل مع الإدارة إن احتجت السحب.');
            } elseif ($tq_avail < 0) {
                $tq_block_title = t('رصيدك المتاح بالسالب');
                $tq_block_text  = t('عليك ____ لأن قيدا خصم من المتاح أكثر مما فيه — كاسترداد بيع بعد أن تحرر مبلغه. وتسدده أرباحك القادمة تلقائيا، ويفتح السحب حين يبلغ المتاح ____.',
                    array($tq_money(-$tq_avail), $tq_money($tq_min)));
            } else {
                $tq_block_title = t('رصيدك لم يبلغ الحد الأدنى للسحب');
                $tq_block_text  = t('المتاح الآن ____ والحد الأدنى ____، فينقصك ____. والمعلق يتحرر بعد نافذة الاسترداد فينضم إلى المتاح، ويفتح النموذج وحده.',
                    array($tq_money($tq_avail), $tq_money($tq_min), $tq_money($tq_min - $tq_avail)));
            }
        }
        ?>
        <form class="tq-card" id="tq-withdraw" method="post"
            ...
            <?php echo tq_csrf(); ?>
            <div class="tq-card__head"><h2 class="tq-card__title"><?php echo t('طلب سحب'); ?></h2></div>

            <?php if (!$tq_can_withdraw): ?>
                <div class="tq-pastel tq-pastel--peach" role="status" id="tq-withdraw-why"
                     style="padding:var(--tq-space-m);margin-block-end:var(--tq-space-l)">
                    <p class="tq-pastel__title tq-strong" style="margin:0 0 var(--tq-space-xs)"><?php echo html_escape($tq_block_title); ?></p>
                    <p class="tq-pastel__body tq-caption" style="margin:0"><?php echo $tq_block_text; ?></p>
                </div>
            <?php endif; ?>

            <div class="tq-field">
                <label class="tq-field__label" for="tq-amount"><?php echo t('المبلغ بالريال'); ?></label>
                <input class="tq-input" id="tq-amount" type="number" name="withdrawal_amount"
                       min="<?php echo number_format($tq_min / 100, 2, '.', ''); ?>"
                       max="<?php echo number_format(max(0, $tq_avail) / 100, 2, '.', ''); ?>"
                       step="0.01" inputmode="decimal" required
                       <?php echo $tq_can_withdraw ? '' : 'disabled aria-describedby="tq-withdraw-why"'; ?>>
                <span class="tq-field__msg tq-field__hint">
                    <?php if ($tq_can_withdraw): ?>
                        <?php echo t('المتاح الآن ____ والحد الأدنى للسحب ____. والمعلق لا يسحب قبل أن يتحرر.', array($tq_money($tq_avail), $tq_money($tq_min))); ?>
                    <?php else: ?>
                        <?php echo t('يفتح الحقل وحده متى زال السبب المكتوب أعلاه.'); ?>
                    <?php endif; ?>
                </span>
            </div>

            <fieldset style="border:0;padding:0;margin:0 0 var(--tq-space-l)">
                <legend class="tq-field__label" style="padding:0"><?php echo t('قناة التحويل'); ?></legend>
                <?php
                ...
                <?php foreach ($tq_w['channels'] as $tq_key => $tq_c): ?>
                    <?php /* `tq-row` يلتف افتراضا، فالتسمية الطويلة («انستاباي — عنوان
                             الدفع…») تنزل سطرا كاملا تحت زرها ويبقى الزر وحده في سطر
                             فارغ. فالصف لا يلتف، والزر في رأس التسمية لا في وسطها. */ ?>
                    <span class="tq-row tq-row--nowrap" style="gap:var(--tq-space-s);margin-block-end:var(--tq-space-s);align-items:flex-start">
                        <input type="radio" id="tq-ch-<?php echo $tq_key; ?>" name="payment_type"
                               value="<?php echo $tq_key; ?>" <?php echo $tq_first ? 'checked' : ''; ?> required
                               data-tq-channel style="flex:none;margin-block-start:.35em"
                               <?php echo $tq_can_withdraw ? '' : 'disabled'; ?>>
                        <label for="tq-ch-<?php echo $tq_key; ?>" style="flex:1 1 auto;min-inline-size:0">
            ...
            <div class="tq-field">
                <label class="tq-field__label" for="tq-dest"><?php echo t('بيانات التحويل'); ?></label>
            ...
            <button class="tq-btn tq-btn--primary tq-btn--block" type="submit"
                    ...
                    <?php echo $tq_can_withdraw ? '' : 'disabled'; ?>>
                <?php echo t('إرسال الطلب'); ?>
            </button>
            <p class="tq-field__msg tq-field__hint" id="tq-withdraw-note" style="margin-block-start:var(--tq-space-m)">
                <?php if (!$tq_can_withdraw): ?>
                    <?php echo t('لا يلزمك شيء الآن: رصيدك وكل قيوده محفوظة في دفترك، وكشف الحساب يبين كل سطر منها.'); ?>
                <?php else: ?>
        ...
        <div class="tq-pastel tq-pastel--sky tq-section">
            <span class="tq-pastel__label tq-micro"><?php echo t('من أين تأتي أرباحك'); ?></span>
            <div class="tq-pastel__body" style="margin:var(--tq-space-s) 0 0">
                <p style="margin:0 0 var(--tq-space-s)">
                    <b><?php echo t('بيع مباشر'); ?></b> <?php echo t('— طالب يشتري كورسك أو مسارك وحده. المبلغ كله عن محتواك، وحصتك منه نسبتك المضبوطة على ذلك المسار.'); ?>
                </p>
                <p style="margin:0 0 var(--tq-space-s)">
                    <b><?php echo t('بيع باقة'); ?></b> <?php echo t('— طالب يشتري باقة تفتح صفوفا كاملة، فيها محتواك ومحتوى زملائك. والباقة تقسم قسمتين:'); ?>
                </p>
                <ol style="margin:0 0 var(--tq-space-s);padding-inline-start:var(--tq-space-l);list-style:decimal">
                    ...
                    <li>
                        <?php echo t('يقسم الوعاء على معلمي الباقة'); ?> <b><?php echo t('بعدد دروسهم'); ?></b> <?php echo t('لا بعدد كورساتهم — فمن له خمسون درسا من مئتين يأخذ ربع الوعاء.'); ?>
                    </li>
                </ol>
                <p style="margin:0">
                    <?php echo t('ومجموع حصص المعلمين يساوي الوعاء بالضبط، فدخول معلم جديد لا يزيد ما تدفعه المنصة ولا ينقص وعاء الباقة — يقسمه على عدد أكبر. وكل سطر باقة في كشفك أعلاه يحمل تحته سعرها ووعاءها وعدد دروسك فيها.'); ?>
                </p>
            </div>
        </div>
