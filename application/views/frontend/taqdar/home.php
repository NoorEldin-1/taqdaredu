...
$tq_why = array(
    ...
    array('i-badge',   'why_5_t', 'نتائج حقيقية',          'why_5_d', 'تابع التقدم ونتائج الاختبارات'),
);
foreach ($tq_why as $w):
?>
...
<!-- ══════════ كتب تقدر في مكتبة جرير ══════════ -->
<?php /* لوحة إعلانية صورة واحدة، تلي «لماذا تختار» بقرار المالك
        (٢٠٢٦-٠٩-١٩). لا نص في القالب: اللوحة تحمل كلامها كاملا،
        فتكراره هنا يزدوج على قارئ الشاشة — والوصف في `alt` وحده.

        وهي صورة واحدة بعرضها الطبيعي على كل المقاسات بقرار المالك:
        لا نسخة جوال مقسومة. و`width/height` يحجزان النسبة قبل
        التحميل فلا تقفز الصفحة. */ ?>
<section class="section" id="books-soon">
  <div class="shell">
    <div class="tqbk reveal">
      <img src="<?php echo tq_site_asset('img/books-jarir.webp'); ?>"
           width="1942" height="809" loading="lazy" decoding="async"
           alt="كتب منصة تقدر قريبا في الأسواق: الدراسات الاجتماعية واللغة الإنجليزية والعلوم والرياضيات واللغة العربية للمرحلة الابتدائية، متوفرة في مكتبة جرير في المملكة العربية السعودية خلال الفترة القادمة إن شاء الله.">
    </div>
  </div>
</section>
...
/* لا بديل مخبوز: كان هنا ثلاث شهادات مخترعة تظهر حين يفرغ الجدول،
   فتفريغه لم يكن يغيّر الصفحة. والقسم كله يختفي الآن حين لا رأي
   منشورًا — وصفحةٌ بلا قسم أصدق من قسمٍ بأسماء لا وجود لها. */
?>
<?php if ($tq_quotes): ?>
        ...
        <article class="quote-card">
          ...
          <div class="quote-card__head">
            <?php /* حرف الاسم في دائرة: الرأي بلا وجه يقرأ أفقر، ووجه
                     مستعار من بنك صور يقرأ تلفيقا. والحرف يميز ولا يدعي. */ ?>
            <?php if (empty($q['avatar'])): ?>
              <span class="quote-card__ini" aria-hidden="true"><?php
                echo html_escape(mb_substr(trim((string) $q['name']), 0, 1, 'UTF-8')); ?></span>
            <?php endif; ?>
