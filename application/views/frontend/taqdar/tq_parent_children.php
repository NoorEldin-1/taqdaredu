<?php
/**
 * بوّابة وليّ الأمر — أبنائي.
 *
 * المرجع التصميمي لهذه البوّابة كلها: تطبيق البنك، لا لوحة تعليمية.
 * كل شيء واضح ومفهوم من نظرة واحدة وبلا مصطلحات: أرقام قليلة كبيرة،
 * وجُمل بلغة الناس، ولا «إتقان تراكمي» ولا «معدّل محطة».
 *
 * الربط بين وليّ الأمر وابنه في `parent_links` (وليّ × ابن × حالة × تاريخ
 * موافقة). وصار يُطلب من هذه الشاشة بدل أن يُنشأ يدويًّا في القاعدة —
 * لكن الطلب يبدأ `pending` بلا تاريخ ولا يفتح شيئًا، ولا يصير `active`
 * إلا بموافقة الابن من حسابه (أو الإدارة) وبتاريخ موثّق في `consent_at`.
 * والتفعيل بلا تاريخ ممنوع في القاعدة نفسها لا في الشيفرة وحدها.
 *
 * ولا يُخمَّن الربط بتشابه اسم أو بريد أو جوّال — خطأ واحد هنا يعني فتح
...
/* النماذج تنشر إلى `POST parent/children/link` في المتحكّم، وهو يستدعي
   دوالّ هذا النموذج نفسها ويكتب سطر تدقيق. وهذا النداء يبقى شبكةَ أمان
...
/* لكل ابن: كورساته ومتوسّط تقدّمه وآخر نشاط — من الجداول الحقيقية. */
                ...
                <?php foreach ($tq_children as $tq_i => $tq_c): ?>
                    ...
                    <article class="tq-card tq-card--float">
                        <div class="tq-row" style="gap:var(--tq-space-l)">
                            ...
                            <div style="flex:1;min-inline-size:0">
                                ...
                                <p class="tq-caption" style="margin:0">
                                    <?php echo tq_iso($tq_c['courses'] . ' مواد مسجّلة'); ?>
                                </p>
                            </div>
                            <?php echo $tq_ok
                                ? tq_badge('mastered', 'نشِط')
                                : tq_badge('due', $tq_c['days'] === null ? 'لم يبدأ بعد' : 'غاب ' . TQ_LRI . $tq_c['days'] . TQ_PDI . ' يومًا'); ?>
                        </div>
                        ...
                        <p class="tq-micro" style="margin:var(--tq-space-m) 0 0">
                            رُبط بموافقة <?php echo html_escape(explode(' ', $tq_name)[0]); ?> بتاريخ
                            <?php echo TQ_LRI . html_escape((string) $tq_c['consent_at']) . TQ_PDI; ?>
                        </p>

                        <form method="post" action="<?php echo base_url('parent/children/link'); ?>"
                              onsubmit="return confirm('إلغاء ربط <?php echo html_escape($tq_name); ?>؟ لن ترى شيئًا من بياناته بعدها، ويلزم طلب جديد وموافقة جديدة منه.');">
        ...
        <?php else: ?>
            ...
            <div class="tq-card tq-empty">
                ...
                <p class="tq-empty__text">
                    بعد الربط ترى في هذه الصفحة صورة واحدة عن كل ابن: ماذا أنهى، وكيف يتقدّم،
                    ومتى كان آخر نشاط له. ولا تُفتح لك بيانات أي طالب قبل أن يوافق هو على الربط.
                </p>
            </div>

        <?php endif; ?>

        <!-- طلبات معلَّقة: أُرسلت ولم يوافق عليها الابن بعد -->
        <?php if ($tq_pending): ?>
            <section class="tq-section" aria-labelledby="tq-pending-h" style="margin-block-start:var(--tq-space-xl)">
                ...
                <div class="tq-card">
                    ...
                    <p class="tq-micro" style="margin-block-start:var(--tq-space-m)">
                        وصلت كلَّ واحد منهم رسالةٌ بنصّ الموافقة وإشعارٌ في حسابه. ولا يُفتح شيء من بياناته قبل أن يوافق هو.
                    </p>
                </div>
            </section>
        <?php endif; ?>
        ...
        <section class="tq-section" aria-labelledby="tq-add-h" style="margin-block-start:var(--tq-space-xl)">
            ...
            <div class="tq-card">
                <form method="post" action="<?php echo base_url('parent/children/link'); ?>">
                    ...
                    <div class="tq-field">
                        <label class="tq-field__label" for="tq-identifier">بريد حساب ابنك في تقدّر (أو رقم حسابه)</label>
                        ...
                        <p class="tq-field__hint">
                            الربط يكون بحساب ابنك في المنصّة نفسها. ولا نبحث بتشابه اسم أو جوّال:
                            خطأ واحد هنا يفتح بيانات طفل لغير أهله.
                        </p>
                    </div>

                    <div class="tq-pastel tq-pastel--sand" style="margin-block:var(--tq-space-l)">
                        <span class="tq-pastel__label tq-micro">نصّ الموافقة الذي يصل ابنك</span>
                    ...
                    <p class="tq-micro" style="margin-block-start:var(--tq-space-m)">
                        يُنشأ الطلب معلّقًا بلا تاريخ موافقة، ولا يفتح شيئًا من بيانات ابنك.
                        وتُسجَّل الموافقة حين يوافق هو من حسابه، بتاريخها ونسخة نصّها ومَن أعطاها.
                    </p>
                </form>
            </div>
        </section>
    ...
    <aside class="tq-aside">
        <div class="tq-pastel tq-pastel--mint">
            ...
            <p class="tq-pastel__body" style="margin:var(--tq-space-s) 0 0">
                تقدّم ابنك ومواده وحصصه ومدفوعاته وملاحظات معلميه.
            </p>
        ...
        <div class="tq-card">
            ...
            <p class="tq-caption">
                يصلك صباح كل أحد: أربعة أسطر تُقرأ في عشر ثوانٍ، تخبرك بما تغيّر هذا الأسبوع.
            </p>
        ...
        <div class="tq-card">
            <div class="tq-card__head"><h2 class="tq-card__title">قبل أن تلغي ربطًا</h2></div>
            <p class="tq-caption">
                إلغاء الربط يُغلق بيانات الابن عنك في الحال، ويبقى في السجلّ تاريخ موافقته
                وتاريخ الإلغاء. وإن ألغيت ربط آخر ابن لك، خرجت من بوّابة وليّ الأمر
