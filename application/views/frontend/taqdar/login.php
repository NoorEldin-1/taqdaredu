...
/* و`?email=` بديل الرابط: «هذا البريد مسجل بالفعل» وروابط البريد تعبئ
   الحقل فلا يعاد كتابته. القيمة تهرب عند الطباعة أدناه كما تهرب أختها. */
if ($tq_old_email === '') {
    $tq_old_email = trim((string) $this->input->get('email'));
}
?>
...
/* TQ-NO-TEACHER-CARD — بطاقة المعلم لا تعرض للعملاء (كما في التسجيل):
   تظهر فقط لمن جاء من `?as=teacher` غير المعلن، فيبقى لوحه مضاء
   ودخوله سليما. */
if ((string) $this->input->get('as') === 'teacher') {
    $tq_gates['teacher'] = array('معلم', 'i-teacher', 'أدرس على المنصة');
}
