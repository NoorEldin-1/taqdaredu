...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        // Support JSON request bodies (Postman / fetch with application/json) across every
        // API controller. CodeIgniter's $this->input->post() only reads $_POST, which PHP
        // fills for form-urlencoded / multipart bodies — NOT raw JSON. Without this, a JSON
        // body arrives empty and fields look "required" (and updates/toggles silently no-op).
        // Merge the decoded JSON into $_POST so all existing $this->input->post() calls work
        // for both body types. php://input stays intact for webhooks that read the raw stream.
        $ctype = $this->input->server('CONTENT_TYPE') ?: ($this->input->server('HTTP_CONTENT_TYPE') ?: '');
        // Parse a JSON body when the client declares application/json, OR as a fallback when
        // $_POST is empty (some clients/apps send a JSON body without the correct Content-Type
        // header — without this the fields look "required" even though they were sent).
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        log_message('error', 'DEBUG_URI raw=['.$_SERVER['REQUEST_URI'].'] path_info=['.($_SERVER['PATH_INFO'] ?? 'NONE').']'); log_message('error', 'DEBUG_REMAP class=['.get_class($this).'] object_called=['.$object_called.'] method=['.$this->request->method.'] controller_method=['.$controller_method.']');
