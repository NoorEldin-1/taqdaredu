...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // User Info Watermark
        <?php 
        $user_watermark_status = get_frontend_settings('user_watermark_status') ? get_frontend_settings('user_watermark_status') : 'active';
        $current_user = $this->db->get_where('users', ['id' => $this->session->userdata('user_id')])->row_array();
        if ($current_user && $user_watermark_status == 'active'): 
        ?>
        var userInfoContainer = document.querySelector('.video-container');
        var userWatermark;
        var userObfuscatedClass = 'uwm_' + Math.random().toString(36).substr(2, 9);
        
        <?php 
        $show_email = get_frontend_settings('user_watermark_show_email') != 'inactive';
        $show_id = get_frontend_settings('user_watermark_show_id') != 'inactive';
        $user_info_parts = [];
        if ($show_id) {
            $user_info_parts[] = 'ID: ' . $current_user['id'];
        }
        if ($show_email) {
            $user_info_parts[] = $current_user['email'];
        }
        $user_info = implode(' | ', $user_info_parts);
        ?>
        var userInfo = '<?php echo $user_info; ?>';
        
        // Get settings from database
        var userWatermarkOpacity = <?php echo get_frontend_settings('user_watermark_opacity') ? get_frontend_settings('user_watermark_opacity') : '0.15'; ?>;
        var userWatermarkFontSize = '<?php echo get_frontend_settings('user_watermark_font_size') ? get_frontend_settings('user_watermark_font_size') : '14'; ?>px';
        var userWatermarkColor = '<?php echo get_frontend_settings('user_watermark_color') ? get_frontend_settings('user_watermark_color') : '#ffffff'; ?>';
        var userWatermarkSpeed = <?php echo get_frontend_settings('user_watermark_speed') ? get_frontend_settings('user_watermark_speed') : '8000'; ?>;

        function createUserWatermark() {
            if (!userWatermark || !document.body.contains(userWatermark)) {
                userWatermark = document.createElement('div');
                userWatermark.className = userObfuscatedClass;
                userWatermark.style.cssText = `
                    position: absolute;
                    visibility: hidden;
                    top: 50%;
                    left: 50%;
                    opacity: ${userWatermarkOpacity};
                    pointer-events: none;
                    color: ${userWatermarkColor};
                    font-size: ${userWatermarkFontSize};
                    font-weight: bold;
                    ...
                    border-radius: 5px;
                `;
                userWatermark.innerText = userInfo;
                userInfoContainer.appendChild(userWatermark);
            }
        }

        function showUserWatermark() {
            if (userWatermark && document.body.contains(userWatermark)) {
                userWatermark.style.visibility = 'visible';
            }
        }

        function getUserRandomPosition() {
            var containerRect = userInfoContainer.getBoundingClientRect();
            var positions = [
                { x: '20%', y: '20%' },
                { x: '80%', y: '20%' },
                { x: '20%', y: '80%' },
        ...
        function animateUserWatermark() {
            if (userWatermark && document.body.contains(userWatermark)) {
                var position = getUserRandomPosition();
                userWatermark.style.left = position.x;
                userWatermark.style.top = position.y;
                
                var randomOpacityMin = Math.max(0.05, userWatermarkOpacity - 0.03);
                var randomOpacityMax = Math.min(0.5, userWatermarkOpacity + 0.05);
                var randomOpacity = randomOpacityMin + Math.random() * (randomOpacityMax - randomOpacityMin);
                userWatermark.style.opacity = randomOpacity;
            }
        }

        function monitorUserWatermark() {
            if (!userWatermark || !document.body.contains(userWatermark) || 
                userWatermark.style.display === 'none' || 
                parseFloat(userWatermark.style.opacity) < 0.05) {
                
                if (userWatermark && document.body.contains(userWatermark)) {
                    userWatermark.remove();
                }
                createUserWatermark();
                showUserWatermark();
            }
        ...
        createUserWatermark();
        setTimeout(showUserWatermark, 2000);
        setInterval(animateUserWatermark, userWatermarkSpeed);
        setInterval(monitorUserWatermark, 15000);

        // Fullscreen support for user watermark
        function handleFullscreenChange() {
            ...
            if (fullscreenElement) {
                // Entering fullscreen - move watermark to fullscreen element
                if (userWatermark && document.body.contains(userWatermark)) {
                    userWatermark.remove();
                }
                userInfoContainer = fullscreenElement;
                createUserWatermark();
                showUserWatermark();
                // Increase z-index for fullscreen
                if (userWatermark) {
                    userWatermark.style.zIndex = '2147483647';
                }
            } else {
                ...
                userInfoContainer = document.querySelector('.video-container');
                createUserWatermark();
                showUserWatermark();
                if (userWatermark) {
                    userWatermark.style.zIndex = '999';
                }
            }
        }
        ...
        document.addEventListener('keydown', function(e) {
            if ((e.key === 'PrintScreen' || e.keyCode === 44) || 
                ...
                if (userWatermark) {
                    userWatermark.style.opacity = '0.4';
                    userWatermark.style.fontSize = '18px';
                    setTimeout(function() {
                        userWatermark.style.opacity = '0.15';
                        userWatermark.style.fontSize = '14px';
                    }, 3000);
                }
            }
        });
