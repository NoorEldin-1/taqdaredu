...
<div class="row">
    <div class="col-md-7" style="padding: 0;">
        ...
        <?php
            // A5 fix: only surface the gateways the React checkout actually
            // supports (TAP + PayPal). Configuring any other gateway here has no
            // effect on the live SPA checkout, so hide them to avoid confusion.
            $supported_gateways = array('tap', 'paypal');
        ?>
        <?php foreach($payment_gateways as $payment_gateway): ?>
            <?php if(!in_array($payment_gateway['identifier'], $supported_gateways)) continue; ?>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="" action="<?php echo site_url('admin/payment_settings'); ?>" method="post" enctype="multipart/form-data">
                            ...
                            <?php // (array) guards against a gateway row with no keys stored yet —
                                  // foreach(null) is a fatal on PHP 8. ?>
                            <?php foreach((array) json_decode($payment_gateway['keys'], true) as $key => $value): ?>
