<?php if($question_type == 'multiple_choice' || $question_type == 'single_choice'): ?>
    ...
    <div id="mcq_choice_input_options">
        <?php if(isset($question_details)): ?>
            <?php foreach((array) json_decode($question_details['options']) as $key => $option): ?>
...
<?php elseif($question_type == 'fill_in_the_blank'): ?>
    <div class="form-group">
        ...
        <input type="text" class="form-control bootstrap-tag-input" value="<?php if(isset($question_details)){foreach((array) json_decode($question_details['correct_answers']) as $answer){echo $answer.',';}} ?>" id = "correct_answers" name="correct_answers" data-role="tagsinput" style="width: 100%;"/>
