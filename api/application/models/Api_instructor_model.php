...
class Api_instructor_model extends CI_Model
{
	...
	{
		$response = array();
		// H-06 fix: fetch by email first, then verify password with bcrypt/sha1 dual-check
		$query = $this->db->get_where('users', array('email' => $_POST['email'], 'is_instructor' => 1, 'role_id' => 2, 'status' => 1));
		if ($query->num_rows() > 0) {
			$row = $query->row_array();
			...
			if (password_verify($password, $row['password'])) {
				...
				$password_ok = true;
				// Upgrade to bcrypt on successful sha1 login
				$this->db->where('id', $row['id'])->update('users', ['password' => password_hash($password, PASSWORD_BCRYPT, ['cost' => 12])]);
			}
			if ($password_ok) {
				...
				$response['is_instructor'] = $row['is_instructor'];
	...
	public function change_password_post($user_id = "")
	{
		...
		if ($query->num_rows() > 0) {
			...
			// H-06 fix: dual-check legacy sha1 + bcrypt
			$password_ok = false;
			if (password_verify($current_password, $user_details['password'])) {
				$password_ok = true;
			} elseif ($user_details['password'] === sha1($current_password)) {
				$password_ok = true;
			}
			if ($password_ok && $new_password == $confirm_password) {
	...
	/**
	 * Object-level authorization helpers. A course's `user_id` may be a
	 * comma-separated list of co-instructors; `creator` is the original owner.
	 */
	private function instructor_owns_course($course_id, $user_id)
	{
		if (!$course_id || !$user_id) return false;
		$course = $this->db->select('user_id, creator')->get_where('course', ['id' => $course_id])->row_array();
		if (!$course) return false;
		$uid = (int) $user_id;
		$owners = array_map('intval', array_filter(explode(',', (string) $course['user_id']), 'strlen'));
		if (in_array($uid, $owners, true)) return true;
		return (int) $course['creator'] === $uid;
	}

	private function instructor_owns_section($section_id, $user_id)
	{
		$section = $this->db->select('course_id')->get_where('section', ['id' => $section_id])->row_array();
		if (!$section) return false;
		return $this->instructor_owns_course($section['course_id'], $user_id);
	}

	private function instructor_owns_lesson($lesson_id, $user_id)
	{
		$lesson = $this->db->select('course_id, section_id')->get_where('lesson', ['id' => $lesson_id])->row_array();
		if (!$lesson) return false;
		if (!empty($lesson['course_id'])) return $this->instructor_owns_course($lesson['course_id'], $user_id);
		return $this->instructor_owns_section($lesson['section_id'], $user_id);
	}

	private function ownership_denied()
	{
		return ['status' => 403, 'validity' => 0, 'message' => 'You do not have permission to modify this item.'];
	}

	public function update_course_post($course_id = "", $user_id = "")
	{
		$response = array();
		if (!$this->instructor_owns_course($course_id, $user_id)) return $this->ownership_denied();
	...
	{
		$response = array();
		// Must own THIS specific course, not merely "some" course.
		if ($this->instructor_owns_course($course_id, $user_id)) {
	...
	{
		$response = array();
		if (!$this->instructor_owns_section($section_id, $user_id)) return $this->ownership_denied();
	...
	public function add_lesson_post($user_id = "")
	{
		...
		// Authorization: caller must own the target course AND the section
		// must belong to that course.
		if (!$this->instructor_owns_course($this->input->post('course_id'), $user_id)) return $this->ownership_denied();
		$own_sec = $this->db->select('course_id')->get_where('section', ['id' => $this->input->post('section_id')])->row_array();
		if (!$own_sec || (int) $own_sec['course_id'] !== (int) $this->input->post('course_id')) return $this->ownership_denied();
		...
		if ($lesson_type == 'video') {
			...
			if ($attachment_type == 'iframe') {
				...
				if ($_FILES['attachment']['name'] == "") {
					...
					// Reject anything that could be executed by the web server.
					$allowed_ext = ['pdf','doc','docx','ppt','pptx','xls','xlsx','txt','csv','zip','rar','7z','png','jpg','jpeg','gif','webp','mp4','webm','mp3','wav','ogg'];
					if (!in_array($fileExtension, $allowed_ext, true)) {
						$response['message'] = api_phrase('invalid_attachment');
						$response['status'] = 403;
						$response['validity'] = 0;
						return $response;
	...
	public function lesson_all_data_get($lesson_id = "", $user_id = "")
	{
		...
		// H-02 fix: enforce authentication — token must be valid
		// lesson_id comes from GET param (legacy pattern kept for compatibility)
		$lesson_id = (int) ($_GET['lesson_id'] ?? $lesson_id);
		if (!$lesson_id) {
			return ['status' => 400, 'validity' => 0, 'message' => 'lesson_id required'];
		}

		// user_id is set by the parent controller via token parsing
		if (empty($user_id)) {
			return ['status' => 401, 'validity' => 0, 'message' => 'Authentication required'];
		}

		// Fetch the lesson
		$lesson = $this->db->get_where('lesson', ['id' => $lesson_id])->row_array();
		if (!$lesson) {
			return ['status' => 404, 'validity' => 0, 'message' => 'Lesson not found'];
		}

		// H-02: Verify the requesting instructor owns the course that contains this lesson
		$section = $this->db->get_where('section', ['id' => $lesson['section_id']])->row_array();
		if (!$section || !$this->instructor_owns_course($section['course_id'], $user_id)) {
			return $this->ownership_denied();
		}

		$response['lesson_details'] = $lesson;
	...
	public function update_lesson_post($lesson_id = "", $user_id = "")
	{
		...
		$response = array();
		if (!$this->instructor_owns_lesson($lesson_id, $user_id)) return $this->ownership_denied();
	...
	public function sort_post($user_id = "", $type = "")
	{
		...
		// Every id in the reorder payload must belong to the caller.
		$ids = json_decode($item_json, true);
		if (!is_array($ids)) return $this->ownership_denied();
		foreach ($ids as $id) {
			$owned = ($type == "section")
				? $this->instructor_owns_section($id, $user_id)
				: $this->instructor_owns_lesson($id, $user_id);
			if (!$owned) return $this->ownership_denied();
		}
		if ($type == "section") {
	...
	public function details_of_sales_report_get($payment_id = "", $user_id = "")
	{
		$response = array();
		...
		// Authorization: the payment's course must belong to this instructor.
		if (!$payment_details || !$this->instructor_owns_course($payment_details['course_id'], $user_id)) {
			return ['status' => 403, 'validity' => 0, 'message' => 'You do not have permission to view this record.'];
		}
