...
class Crud_model extends CI_Model
{
    ...
    function get_course_thumbnail_url($course_id, $type = 'course_thumbnail')
    {
        ...
        $optimized_base = 'uploads/thumbnails/course_thumbnails/optimized/' . $type . '_' . get_frontend_settings('theme') . '_' . $course_id . $last_modified;
        $original_file = 'uploads/thumbnails/course_thumbnails/' . $type . '_' . get_frontend_settings('theme') . '_' . $course_id . $last_modified . '.jpg';

        if (file_exists($optimized_base . '.webp')) {
            return base_url() . $optimized_base . '.webp';
        } elseif (file_exists($optimized_base . '.jpg')) {
            return base_url() . $optimized_base . '.jpg';
        } elseif (file_exists($original_file)) {

            //resizeImage
            resizeImage($original_file, 'uploads/thumbnails/course_thumbnails/optimized/', 400);

            if (file_exists($optimized_base . '.jpg')) {
                return base_url() . $optimized_base . '.jpg';
            }
            return base_url() . $original_file;
        } else {
    ...
    public function get_courses_by_search_string($search_string = "", $per_page = "", $uri_segment = "")
    {
        ...
        // Show active courses first in search results
    ...
    public function update_water_mark()
    {
        ...
        $user_watermark_status = $this->input->post('user_watermark_status');
        if ($user_watermark_status) {
            $data['value'] = $user_watermark_status;
            $this->db->where('key', 'user_watermark_status');
            if ($this->db->get_where('frontend_settings', ['key' => 'user_watermark_status'])->num_rows() > 0) {
                $this->db->where('key', 'user_watermark_status');
                $this->db->update('frontend_settings', $data);
            } else {
                $data['key'] = 'user_watermark_status';
                $this->db->insert('frontend_settings', $data);
            }
        }

        $user_watermark_speed = $this->input->post('user_watermark_speed');
        if ($user_watermark_speed) {
            $data['value'] = $user_watermark_speed;
            $this->db->where('key', 'user_watermark_speed');
            if ($this->db->get_where('frontend_settings', ['key' => 'user_watermark_speed'])->num_rows() > 0) {
                $this->db->where('key', 'user_watermark_speed');
                $this->db->update('frontend_settings', $data);
            } else {
                $data['key'] = 'user_watermark_speed';
                $this->db->insert('frontend_settings', $data);
            }
        }

        $user_watermark_opacity = $this->input->post('user_watermark_opacity');
        if ($user_watermark_opacity) {
            $data['value'] = $user_watermark_opacity;
            $this->db->where('key', 'user_watermark_opacity');
            if ($this->db->get_where('frontend_settings', ['key' => 'user_watermark_opacity'])->num_rows() > 0) {
                $this->db->where('key', 'user_watermark_opacity');
                $this->db->update('frontend_settings', $data);
            } else {
                $data['key'] = 'user_watermark_opacity';
                $this->db->insert('frontend_settings', $data);
            }
        }

        $user_watermark_font_size = $this->input->post('user_watermark_font_size');
        if ($user_watermark_font_size) {
            $data['value'] = $user_watermark_font_size;
            $this->db->where('key', 'user_watermark_font_size');
            if ($this->db->get_where('frontend_settings', ['key' => 'user_watermark_font_size'])->num_rows() > 0) {
                $this->db->where('key', 'user_watermark_font_size');
                $this->db->update('frontend_settings', $data);
            } else {
                $data['key'] = 'user_watermark_font_size';
                $this->db->insert('frontend_settings', $data);
            }
        }

        $user_watermark_color = $this->input->post('user_watermark_color');
        if ($user_watermark_color) {
            $data['value'] = $user_watermark_color;
            $this->db->where('key', 'user_watermark_color');
            if ($this->db->get_where('frontend_settings', ['key' => 'user_watermark_color'])->num_rows() > 0) {
                $this->db->where('key', 'user_watermark_color');
                $this->db->update('frontend_settings', $data);
            } else {
                $data['key'] = 'user_watermark_color';
                $this->db->insert('frontend_settings', $data);
            }
        }

        $user_watermark_show_email = $this->input->post('user_watermark_show_email') ? 'active' : 'inactive';
        $data['value'] = $user_watermark_show_email;
        $this->db->where('key', 'user_watermark_show_email');
        if ($this->db->get_where('frontend_settings', ['key' => 'user_watermark_show_email'])->num_rows() > 0) {
            $this->db->where('key', 'user_watermark_show_email');
            $this->db->update('frontend_settings', $data);
        } else {
            $data['key'] = 'user_watermark_show_email';
            $this->db->insert('frontend_settings', $data);
        }

        $user_watermark_show_id = $this->input->post('user_watermark_show_id') ? 'active' : 'inactive';
        $data['value'] = $user_watermark_show_id;
        $this->db->where('key', 'user_watermark_show_id');
        if ($this->db->get_where('frontend_settings', ['key' => 'user_watermark_show_id'])->num_rows() > 0) {
            $this->db->where('key', 'user_watermark_show_id');
            $this->db->update('frontend_settings', $data);
        } else {
            $data['key'] = 'user_watermark_show_id';
            $this->db->insert('frontend_settings', $data);
        }
    }
    ...
    public function get_courses_by_wishlists()
    {
        ...
        if (is_array($wishlists) && sizeof($wishlists) > 0) {
            ...
            // Show active courses first in wishlist
            $this->db->order_by("FIELD(status, 'active', 'upcoming', 'pending', 'draft')", '', FALSE);
            $this->db->order_by('id', 'desc');
            return $this->db->get('course')->result_array();
        } else {
            return array();
    ...
    public function get_courses_of_wishlists_by_search_string($search_string)
    {
        ...
        if (sizeof($wishlists) > 0) {
            $this->db->where_in('id', $wishlists);
            $this->db->like('title', $search_string);
            // Show active courses first in wishlist search
            $this->db->order_by("FIELD(status, 'active', 'upcoming', 'pending', 'draft')", '', FALSE);
            $this->db->order_by('id', 'desc');
            return $this->db->get('course')->result_array();
        } else {
    ...
    function curl_request($code = '')
    {
        ...
        // !empty() not count(): the Envato call returns nothing when the network is
        // unreachable or the code is unrecognised, and count(null) is fatal on PHP 8.
        // The existing else branch already reports the licence as invalid.
        if (!empty($response['verify-purchase'])) {
            return true;
        } else {
            return false;
        }
    ...
    function filter_course($search_string = "", $selected_category_id = "", $selected_price = "", $selected_level = "", $selected_language = "", $selected_rating = "", $selected_sorting = "", $per_page = "", $uri_segment = "")
    {
        ...
        if ($selected_rating != "all") {
            ...
            $this->db->or_where('c.status', 'upcoming');
            $this->db->group_end();

            $this->db->from('course c')->join('rating r', 'r.ratable_id = c.id', 'left');

            $this->db->group_by('c.id');
            //show active courses first, then by rating
        ...
        //sorting: always show active courses first
        $this->db->order_by("FIELD(c.status, 'active', 'upcoming')", '', FALSE);
    ...
    public function add_coupon()
    {
        if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && !empty($_POST['discount_percentage']) && isset($_POST['expiry_date']) && !empty($_POST['expiry_date'])) {
            ...
            // Empty/"0" course_id means the coupon applies store-wide (NULL).
            $course_id = $this->input->post('course_id');
            $data['course_id'] = !empty($course_id) ? (int) $course_id : null;
    ...
    {
        // C-03 fix: cryptographically secure token — not guessable
        $email = $this->input->post('email');

        // Always show the same response to prevent account enumeration (M-03)
        ...
        // random_bytes(32) = 64 hex chars, 2^256 possibilities
    ...
    {
        // C-03 fix: verify token exists, not expired, then invalidate it (one-time use)
        if (empty($verification_code)) {
            return false;
        }

        // Token must match a real user AND not be expired
        $user = $this->db
        ...
        $new_invalidation_token = bin2hex(random_bytes(32));
        $this->db->where('id', $user['id']);
        $this->db->update('users', [
            'password'          => $hashed,
            'verification_code' => $new_invalidation_token, // invalidate old token
            'reset_expiry'      => 0,                       // force expiry
            'last_modified'     => time()
        ]);
        return true;
    }
    ...
    public function get_courses_by_instructor_id($instructor_id = 0, $status = "")
    {
        ...
        // Show active courses first in instructor page
        $this->db->order_by("FIELD(status, 'active', 'upcoming', 'pending', 'draft')", '', FALSE);
        $this->db->order_by('id', 'desc');
