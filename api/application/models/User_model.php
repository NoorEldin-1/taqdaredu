...
class User_model extends CI_Model
{
    ...
    public function add_user($is_instructor = false, $is_admin = false)
    {
        ...
        if ($validity == false) {
            ...
            $data['password'] = password_hash(html_escape($this->input->post('password')), PASSWORD_BCRYPT, ['cost' => 12]);
    ...
    {
        $user = $this->db->get_where('users', array('id' => $this->session->userdata('user_id')))->row_array();
        if (!$user) return 0;

        // H-06 fix: dual-check for backward compatibility during migration
        $password_ok = false;
    ...
    public function update_account_settings($user_id)
    {
        ...
        if ($validity) {
            if (!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['confirm_password'])) {
                ...
                // H-06 fix: dual-check legacy sha1 support + bcrypt upgrade
                $password_ok = false;
    ...
    public function get_user_image_url($user_id)
    {
        ...
        $optimized_base = 'uploads/user_image/optimized/' . $user_profile_image;
        $original_file = 'uploads/user_image/' . $user_profile_image . '.jpg';
        ...
        if (file_exists($optimized_base . '.webp')) {
            ...
            //resizeImage - this helper only does resize. We might need a smarter conversion if possible, 
            // but for now let's rely on the python script for mass conversion, and fallback to resize-only for new uploads.
            // Ideally core PHP would do webp conversion too but that might require GD/Imagick updates.
            resizeImage($original_file, 'uploads/user_image/optimized/', 220);
            return base_url() . $optimized_base . '.jpg';
        } else {
