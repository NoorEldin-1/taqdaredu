...
class Login extends CI_Controller
{
    ...
    public function validate_login($from = "")
    {
        ...
        // A1 fix: brute-force protection for the web login (admins log in here).
        // Reuses the same login_attempts table + policy as the JWT API login.
        ...
        if ($recent_failures >= $max_attempts) {
            $this->session->set_flashdata('error_message', get_phrase('too_many_login_attempts') ?: 'Too many failed login attempts. Please try again in about 15 minutes.');
            redirect(site_url('login'), 'refresh');
            return;
        }
        ...
        // H-06 fix: dual-check bcrypt/sha1
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($password_ok) {
                // Clear this IP's failed attempts on success.
            ...
            // Record the failed attempt even for unknown emails (blocks enumeration-driven guessing).
            $this->db->insert('login_attempts', ['ip' => $ip, 'email' => $email, 'attempted_at' => $now]);
            $this->session->set_flashdata('error_message', get_phrase('invalid_login_credentials'));
            redirect(site_url('login'), 'refresh');
        }
    ...
    public function google_callback()
    {
        ...
        if (isset($_GET['code'])) {
            ...
            try {
                ...
                $client->setClientId(get_settings('google_client_id'));
                $client->setClientSecret(get_settings('google_client_secret'));
                $client->setRedirectUri(get_settings('google_redirect_uri'));
                ...
                $payload = array(
                    ...
                    'email_verified' => $user_info->verifiedEmail,
                    'given_name' => $user_info->givenName,
                    'family_name' => $user_info->familyName
                );

                $this->process_google_login($payload);
            ...
            $this->session->set_flashdata('error_message', get_phrase('no_authorization_code_received'));
            redirect(site_url('login'), 'refresh');
        }
    }
    ...
    public function google_validate_login()
    {
        ...
        if (empty($id_token)) {
            echo json_encode(['success' => false, 'message' => get_phrase('invalid_token')]);
            return;
        }

        $this->social_login_modal->google_validate_login($id_token);
    }

    private function process_google_login($payload)
    {
        $email = $payload['email'];
        ...
        // Also set CI session for legacy compatibility
        $this->user_model->new_device_login_tracker($user_id);
        $this->session->set_userdata('user_id', $user_id);
        $this->session->set_userdata('user_login', '1');

        // Generate JWT token and redirect to React frontend
        ...
        $userdata['token'] = $tokenHandler->GenerateToken($userdata);

        // A6 fix: pass auth data in the URL *fragment* (#), not the query (?).
        // A fragment is never sent to the server, so the JWT no longer lands in
        // web-server / proxy access logs or the Referer header.
        $auth_data = base64_encode(json_encode($userdata));
        $react_url = 'https://my-communication.uk/login#google_auth=' . $auth_data;
