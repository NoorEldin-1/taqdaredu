...
class Home extends CI_Controller
{
    ...
    public function courses()
    {
        ...
        if ($search_string == "" && $selected_category_id == "all" && $selected_price == "all" && $selected_level == 'all' && $selected_language == 'all' && $selected_rating == 'all' && $selected_sorting == 'newest') {
            ...
            $this->db->or_where('status', 'upcoming');
            $this->db->group_end();
            ...
            $this->db->or_where('status', 'upcoming');
            $this->db->group_end();
            //sorting: active courses first, then by newest
    ...
    public function course($slug = "", $course_id = "")
    {
        ...
        // Legacy theme URL — 301-redirect to the canonical SPA course page so old
        // shared links and crawlers consolidate on /courses/<id> (clean meta + OG).
        // The affiliate referral above has already been saved to the session, so
        // redirecting here keeps affiliate attribution intact.
        $cid = (int) $course_id;
        if ($cid > 0) {
            $root = rtrim(get_settings('system_url') ?: 'https://my-communication.uk', '/');
            header('Location: ' . $root . '/courses/' . $cid, true, 301);
            exit;
        }
