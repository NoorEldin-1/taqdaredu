...
class Payment extends CI_Controller
{
     ...
     * Called via AJAX from payment form
     */
    public function create_tap_charge()
    {
        // Load Tap payment model
        ...
        if (!$payment_details) {
            echo json_encode([
                'success' => false,
                'message' => get_phrase('invalid_payment_session')
            ]);
            return;
        }

        // Determine payment type
        $type = ($payment_details['is_instructor_payout_user_id'] > 0) ? 'instructor_payout' : 'course';
        
        // Create charge
        $charge_response = $this->tap_payment_model->create_tap_charge($identifier, $type);
        
        if ($charge_response && isset($charge_response['transaction']['url'])) {
            // Return redirect URL to hosted payment page
            echo json_encode([
                'success' => true,
                'redirect_url' => $charge_response['transaction']['url'],
                'charge_id' => $charge_response['id']
            ]);
        } else {
            echo json_encode([
                'success' => false,
                'message' => isset($charge_response['message']) ? $charge_response['message'] : get_phrase('payment_initialization_failed')
            ]);
        }
    }
     ...
     * Called by Tap servers when payment status changes
     */
    public function tap_webhook()
    {
        // Load Tap payment model
        $this->load->model('tap_payment_model');
        ...
        // Process webhook
        $result = $this->tap_payment_model->process_tap_webhook($webhook_data);
    ...
    /**
     * Tap payment redirect handler
     * User is redirected here after completing payment on Tap hosted page
     */
    public function tap_redirect()
    {
        // This is handled by the generic success_course_payment() or success_instructor_payout()
        // which will call check_tap_payment() from the model
        
        // Get tap_id from URL and add to session if not already there
        $tap_id = $this->input->get('tap_id');
        if ($tap_id && !$this->session->userdata('tap_charge_id')) {
            $this->session->set_userdata('tap_charge_id', $tap_id);
        }
    }
}
