...
class Blog extends CI_Controller
{
    ...
    function details($blog_slug = "", $blog_id = ""){
        // Legacy theme URL — 301-redirect to the canonical SPA blog page so old
        // shared links, admin "view" links and crawlers all consolidate on
        // /blogs/<id> (clean meta + Open Graph; no raw-HTML description leaking
        // from the old theme).
        $bid = (int) $blog_id;
        if ($bid > 0) {
            $root = rtrim(get_settings('system_url') ?: 'https://my-communication.uk', '/');
            header('Location: ' . $root . '/blogs/' . $bid, true, 301);
            exit;
        }
