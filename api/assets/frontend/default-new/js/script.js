...
$(document).ready(function () {
  $(".menu_pro_cart_tgl").click(function () {
    $(".menu_pro_cart").toggleClass("active");
  });
});
$(document).ready(function () {
...
if (el) {
  /* PERFORMANCE OPTIMIZATION: Cache dimensions to avoid forced reflows */
  let cachedHeight = el.clientHeight;
  let cachedWidth = el.clientWidth;

  /**
   * Update cached dimensions on resize
   * Uses debounced requestAnimationFrame to prevent layout thrashing
   */
  let resizeTimeout;
  ...
  function handleMove(e) {
     ...
     * OPTIMIZATION: Uses cached width instead of reading clientWidth
     */
    const yRotation = 20 * ((xVal - cachedWidth / 2) / cachedWidth);

    /* Calculate the rotation along the X-axis
     * OPTIMIZATION: Uses cached height instead of reading clientHeight
     */
    const xRotation = -20 * ((yVal - cachedHeight / 2) / cachedHeight);
...
//  slider
// PERFORMANCE OPTIMIZATION: Carousel initialization moved to lazy_carousel_init.js
// to eliminate forced reflows. Carousels now use Intersection Observer for lazy loading.
...
// PERFORMANCE OPTIMIZATION: All Slick carousel initialization moved to lazy_carousel_init.js
// Commented out to prevent duplicate initialization and reduce forced reflows
/*
// Slick Carousel
